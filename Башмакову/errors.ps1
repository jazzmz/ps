###########################################################################
### Программа: Скрипт для обработки ответов БИС-а		###########
### Автор: Доркин Д.В.						###########
...
$path_with_files = "C:\test\Башмакову\"			#Папка где лежат исходные файлы
...
del $text_to_send
	echo ---------------------------------------------------------------- >> $text_to_send
dir -Filter UNIFO*xml -Name $path_with_files | % {$i = $i + 1}
echo "Было выложено $i XML файлов" >> $text_to_send
	echo ---------------------------------------------------------------- >> $text_to_send
dir -Filter UNIFO*xml -Name $path_with_files | % { # echo $_ 

$line_ish = Get-Content $_ # Исходная линия ХЭМЭЛЬника для работы

$line = $line_ish -split '<ErrorCode>' # Индийский код вырезания номера ошибки
$line = $line[1] -split '</ErrorCode>'
$errorcode = $line[0]

if ($errorcode -ne "") { #Если код ошибки-таки есть. По идее это стоило проверить еще на этапе первого отсечения.
	echo "Код ошибки в файле $_ равен $errorcode"			      >> $text_to_send
	echo ---------------------------------------------------------------- >> $text_to_send
			};
...
#notepad $text_to_send
cmd /c start $blat_exe $text_to_send -tf $addresses -subject "Отчет об обработке файлов"
};
