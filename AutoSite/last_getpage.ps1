...
$sendfile="$workdir\sendfile"
$PSEmailServer="mail.pirbank.ru"
$utf8=[System.Text.Encoding]::UTF8
$to1="bis@pirbank.ru"
$to2="u6@pirbank.ru" 
$to3="u10_1@pirbank.ru"
#$to="ddorkin@pirbank.ru"
$subject="Information about 365 form downloading"
$from="bis@pirbank.ru"
$savedir="O:\F365P\Rashifr\Temp"
$global:anyflag=$false
#$global:list=""

function sendmail ([string]$body) {
    #Send-Mailmessage -from $from -to "$to1", "$to2", "$to3" -subject $subject -body $body -encoding $utf8
    Send-Mailmessage -from $from -to "$to1" -subject $subject -body $body -encoding $utf8
}
...
$url2contentSel | % {
    ...
    if ($count -eq 0) {
        ...
        else
        {
            ...
            echo $numforlog >>$sendfile
            echo "`n" >>$sendfile
            #$global:list=$global:list + "`n" + $numforlog
        }
        $global:anyflag=$true
...
#Remove-Variable ie
Get-Date
$flag=Test-Path $sendfile
if ((!$anyflag) -and (!$flag)){
    sendmail "365 форма отсутствует"
    echo "flag=$anyflag"
}
else
{
    $list = Get-Content $sendfile
    sendmail $list
    rm $sendfile
}
sleep 1

#start-process "command.com /c T:\Operu\F365P\Rashifr\365rashifr.bat"
