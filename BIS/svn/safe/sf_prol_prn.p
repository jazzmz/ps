{pirsavelog.p}
/**	
 * Печать распоряжения по оплате за новый срок аренды (пролонгация).
 * Бурягин Е.П.
 * 27.03.2006 9:00
 */
  ...
  * Сначала ищем документы с корреспонденцией 408* - 60311. Затем ищем их "пары".
 ...
 * из параметра iParam и с корреспонденцией 408* - 60311.
 */
FOR EACH tmprecid NO-LOCK,
    ...
    FIRST bfr-op-entry WHERE
     	bfr-op-entry.acct-db = op-entry.acct-cr
     	AND
     	...
     	NO-LOCK
:
		/** Если корреспонденция счетов соответствует 408* - 60311, то */
		IF op-entry.acct-db BEGINS "408" AND op-entry.acct-cr BEGINS "60311" THEN DO:
			/** Найдем договор */
...
PUT UNFORMATTED SPACE(70) "В Управление Пластиковых Карт" SKIP
                ...
                SPACE(28) 'О СПИСАНИИ ДЕНЕЖНЫХ СРЕДСТВ' SKIP
                SPACE(33) 'ОПЛАТА ЗА ЯЧЕЙКУ' SKIP 
