...
/**	
 * Печать распоряжения о списании денежных средств.
 * Бурягин Е.П.
 * 18.05.2006 9:05
 */
...
:
			FIND FIRST loan-acct WHERE 
				...
				AND
				CAN-DO("ДепСейфПлКрт,ДепСейфНСОст",loan-acct.acct-type)
				NO-LOCK NO-ERROR.
...
PUT UNFORMATTED SPACE(0) ENTRY(3,iParam,";") SKIP
                ...
                SPACE(35) 'РАСПОРЯЖЕНИЕ О СПИСАНИИ ДЕНЕЖНЫХ СРЕДСТВ' SKIP
                SPACE(23) 'Согласно Тарифам, установленным в ' cBankName SKIP 
                SPACE(23) '             удержать комиссию со следующих лиц' SKIP(3).

/** Заголовки */
PUT UNFORMATTED 
	 	"┌─────────────┬────────────────────┬────────────────────┬────────────────────┬───────────────────┬──────────────────────────────┐" SKIP
		"│№            │Ф.И.О.              │Счет                │Счет                │Сумма согласно     │Назначение                    │" SKIP
		"│дог          │                    │дебета              │кредита             │Тарифов (рубл.)    │платежа                       │" SKIP
	 	"├─────────────┼────────────────────┼────────────────────┼────────────────────┼───────────────────┼──────────────────────────────┤" SKIP.
