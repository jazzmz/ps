...
/*****************************************************************
 * ПО ЗАЯВКЕ #1317 ВЫПИСКА В РЕЗУЛЬТАТЕ ИСПРАВИТЕЛЬНОЙ ПРОВОДКИ. *
 *****************************************************************/
...
DEF VAR taxon   AS CHAR    INIT "fin ved acct storno" NO-UNDO.
...
FOR EACH acct WHERE CAN-DO(GetFltVal('acct'), acct.acct) 
                AND CAN-DO(GetFltVal('acct-cat'),acct.acct-cat)
...
FIND FIRST user-config WHERE
        user-config.user-id    = entry(1,iFlt)        AND 
        user-config.descr      = ENTRY(4,iFlt)        AND
        user-config.proc-name  = "stmt(dd).p"         AND 
        user-config.sub-code   = "stmt"               
        NO-LOCK NO-ERROR.
