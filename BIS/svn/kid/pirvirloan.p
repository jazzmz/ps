...
/**
 *
 * Расчет планируемых расходов по будущим начислениям процентов по кредитным договорам
 * Ермилов В.Н.
 * 30/03/2010
 *
 **/
...
DEF VAR oSysClass AS TSysClass.         /* Подключаем библиотеку функций */

FUNCTION NOMER_YACH INTEGER
   (INPUT dDateBeg AS DATE,
    INPUT dDateEnd AS DATE):

   oSysClass = new TSysClass().

   DO WHILE (WEEKDAY(dDateEnd) EQ 7 OR WEEKDAY(dDateEnd) EQ 0):
      dDateEnd = dDateEnd - 1.
   END.

   RETURN oSysClass:getLineSegment(dDateEnd - dDateBeg,"1,4,5,10,10,60,90,90,95").
...
. 
	...
	/* Счет для перечисления процентов */
	tt-report.intAccount = GetLoanAcct_ULL(loan.contract, loan.cont-code, "КредТ", repDate, traceOnOff).
		
  	/* Счет договора */
  	tt-report.account = GetLoanAcct_ULL(loan.contract, loan.cont-code, "Кредит", repDate, traceOnOff).
	...
	/* Остаток на кредитном счете  */	
	tt-report.summaKredit = ABS(	GetAcctPosValue_UAL(tt-report.account,	loan.currency,tmpDate, traceOnOff)	).

	/* Действующая ставка по кредиту */		
	tt-report.rate = GetLoanCommission_ULL(loan.contract,loan.cont-code,"%Кред",repDate, traceOnOff).
...
/* находим следующую для данного договора дату выплаты процентов */
   nextDate = GetLoanNextDatePercentPayOut_ULL(loan.contract, loan.cont-code, repDate, TraceOnOff).
...
/* MESSAGE 'Dati oplati&nachisleniya: 'tekdate  nextDate VIEW-AS ALERT-BOX. */ 
...
/*    MESSAGE 'First Percent: ' firstPercent VIEW-AS ALERT-BOX.  */
		...
		tt-report.Pr4period[n_yach] = tt-report.Pr4period[n_yach] + nextPercent . 
...
/*Теперь учтем проценты расчитываемые в день закрытия кредитного договора */
...
FOR EACH tt-report BREAK BY balAcct%% BY tt-report.currency :
	...
	PUT UNFORMATTED
		...
		"|" tt-report.summaKredit FORMAT "->>>,>>>,>>9.99"
...
FUNCTION PIR_LOAN_PERCENT RETURNS DECIMAL (INPUT iContract AS CHAR,INPUT iContCode AS CHAR, INPUT iBegDate AS DATE, INPUT iEndDate AS DATE, INPUT iNeedMon AS LOGICAL):   
   ...
   DEF VAR balance AS DECIMAL INITIAL ? NO-UNDO.
   ...
   */
   ...
   DO iDate = periodBegin TO periodEnd :
 	...
 	*/
     ...
     IF  (balance <> newBalance) OR (rate <> newRate) OR (DAY(iDate + 1) = 1 AND iDate < periodEnd) OR iDate = periodEnd THEN
       DO:
