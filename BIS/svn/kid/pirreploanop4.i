/*
		Погашение штрафов.
		Четвертый раздел отчета по операциям по кредитным договорам.
		Бурягин Е.П., 16.01.2006 15:16
*/
...
FOR EACH tmprecid 
   ...
   FIRST op-entry OF op WHERE
         NO-LOCK
: 
	/* Операция по погашению процентов по кредиту - это операция под номером 57 и 358.
	...
	*/
	IF op-entry.kau-cr <> "" THEN
		IF NUM-ENTRIES(op-entry.kau-cr) = 3 THEN
			IF ENTRY(1,op-entry.kau-cr) = "Кредит" AND CAN-DO("358,57",ENTRY(3, op-entry.kau-cr)) THEN
				DO:
...
PUT UNFORMATTED	
	"  4) П О Г А Ш Е Н И Е   Ш Т Р А Ф О В" SKIP
	"┌──────┬──────────────────────────────┬───────────┬──────────┬──────┬────────────────────┬────────────────────┬──────────────────┬───┬──────────────────────────────┬───────────┬─────────┬─────────────┐" SKIP.
...
PUT UNFORMATTED	
	"│      │                              │           │          │      │                    │      ИТОГО(штрафы):│" subtotal_amt_rub FORMAT ">>>,>>>,>>>,>>9.99" 
																																																																   "│810│                              │           │         │             │" SKIP 
