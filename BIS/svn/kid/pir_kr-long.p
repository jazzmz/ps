...
/*
   Отчет "Пролонгированная ссудная задолженность"
   ТЗ Красновой
   Борисов А.В. 20.10.2009
*/
...
"            Пролонгированная ссудная задолженность на " STRING(end-date) SKIP(2)
"┌─────┬───────────────────────────────────┬──────────────┬───────────────────┐" SKIP
"│  №  │       НАИМЕНОВАНИЕ ЗАЕМЩИКА       │ № КРЕДИТНОГО │   СУММА ССУДНОЙ   │" SKIP
"│ П/П │                                   │   ДОГОВОРА   │   ЗАДОЛЖЕННОСТИ   │" SKIP
"├─────┴───────────────────────────────────┴──────────────┴───────────────────┤" SKIP
.
...
DO J = 1 TO 3:
   ...
   FOR EACH loan
      WHERE (loan.currency EQ cCurr[J])
        ...
        AND NOT (loan.cont-code BEGINS "ПК")
      ...
      NO-LOCK,
      LAST pro-obl
         WHERE (pro-obl.cont-code EQ loan.cont-code)
           AND (pro-obl.idnt EQ 3)
           AND (pro-obl.pr-date LE end-date)
           AND (pro-obl.n-end-date GE end-date)
         NO-LOCK
      BY loan.cont-code:
