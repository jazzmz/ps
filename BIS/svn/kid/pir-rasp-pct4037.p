...
/* 2  */  oTable1:addCell("Наименование заемщика").
/* 3  */  oTable1:addCell("Качество задолженности").
/* 4  */  oTable1:addCell("Сумма задолженности").
    ...
    DO:
       ...
       DO:
	      ...
	      FIND FIRST cust-corp WHERE cust-corp.cust-id = loan.cust-id NO-LOCK NO-ERROR.
           ...
           CREATE rep.
           ASSIGN
             rep.f02 = ClName + ";" + getMainLoanAttr("Кредит",loan.cont-code,"№ %cont-code от %ДатаСогл")
             rep.f03 = TypeZad
             rep.f04 = dOstKredit
...
HeadTbl =
  ...
  + "и отражения указанных операций по счетам бухгалтерского учета прошу начислить на балансовых счетах проценты на задолженность по следующим кредитам:" + CHR(10) + CHR(10)
...
FOR EACH rep BREAK BY rep.f05 BY rep.f02 BY rep.f03 :
   ...
   cTmp = IF rep.f03 = 1 THEN "Срочный ОД" ELSE "Просроченный ОД".
