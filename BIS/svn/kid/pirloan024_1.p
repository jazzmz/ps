/* по заявке 1002 переделаем на расчет по остатку просроченной ссудной задолженности */
...
FOR FIRST tmprecid NO-LOCK,
    ...
    :
		 ...
		 */
					...
					Balance = GetLoanParamValue_ULL("Кредит",Loan, 7, beg_date - 1, ULLShowErrorMsg).
					PeriodBegin = beg_date.
					PeriodEnd = end_date.
					/** Процентная ставка по кредитру на дату начала периода расчета процентов */
					Rate = GetLoanCommission_ULL("Кредит",Loan,"%КрПр",beg_date, ULLShowErrorMsg).
					...
					DO idate = PeriodBegin TO PeriodEnd - 1:
							NewBalance = GetLoanParamValue_ULL("Кредит", Loan, 7, idate, ULLShowErrorMsg).
							NewRate = GetLoanCommission_ULL("Кредит",Loan, "%КрПр", idate + 1, ULLShowErrorMsg).
  	...
  	SummaStr[1] = SummaStr[1] + ' ' + SummaStr[2].
		...
		MainText[1] = "Списать в безакцептном порядке со счета №" + LoanAcct + " с последующим зачислением "
		            ...
		            + " в счет погашения процентов за пользование просроченным кредитом с " 
