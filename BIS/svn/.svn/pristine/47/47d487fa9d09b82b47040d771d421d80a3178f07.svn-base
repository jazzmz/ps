...
FOR EACH tmprecid NO-LOCK,
   ...
   cXL = XLRow(0)
       ...
       + XLEmptyCells(6)
       + XLCell("Входящий остаток на счете:")
   ...
   FOR EACH op
      ...
      IF LAST-OF(op-entry.op-date)
      THEN DO:
         RUN acct-pos IN h_base(acct.acct, acct.currency, op-entry.op-date, op-entry.op-date, cXL).
         cXL = XLRow(1)
