/*
		Погашение кредитов.
		Второй раздел отчета по операциям по кредитным договорам.
		Бурягин Е.П., 16.01.2006 14:49
*/
...
FOR EACH tmprecid 
   ...
   FIRST op-entry OF op WHERE
         NO-LOCK
: 
	/* Операция по погашению кредита - это операция под номером 5.
	   ...
	   НО 
	   по кредиту субаналитическая проводка не должна быть операцией с кодом 4
	*/
	IF op-entry.kau-cr <> "" THEN
		IF NUM-ENTRIES(op-entry.kau-cr) = 3 THEN
			IF ENTRY(1,op-entry.kau-cr) = "Кредит" AND CAN-DO("5,50",ENTRY(3, op-entry.kau-cr)) THEN
			  IF 	(NUM-ENTRIES(op-entry.kau-db)<3)
			  		OR 
			  		(NUM-ENTRIES(op-entry.kau-db)=3 
			  		AND 
...
PUT UNFORMATTED	
	"  2) П О Г А Ш Е Н И Е   К Р Е Д И Т О В" SKIP
	"┌──────┬──────────────────────────────┬───────────┬──────────┬──────┬────────────────────┬────────────────────┬──────────────────┬───┬──────────────────────────────┬───────────┬─────────┬─────────────┐" SKIP.
...
PUT UNFORMATTED	
	"│      │                              │           │          │      │                    │    ИТОГО(погашено):│" subtotal_amt_rub FORMAT ">>>,>>>,>>>,>>9.99" 
																																																																   "│810│                              │           │         │             │" SKIP 
