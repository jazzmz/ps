/*
		...
		Шестой раздел отчета по операциям по кредитным договорам.
		Бурягин Е.П., 16.01.2006 15:47
*/
...
FOR EACH tmprecid 
   ...
   FIRST op-entry OF op WHERE
         NO-LOCK
: 
	/* Внебалансовые операции - перации по внебалансовым счетам привязанные по дебетовой или 
	   кредитовой аналитике. 
	   Формат записи в этом поле такой: "Кредит,<НОМЕР_ДОГОВОРА>,<НОМЕР_ОПЕРАЦИИ>".
	*/
	IF op-entry.acct-cat = "o" AND (
	...
	THEN
		/* Может пригодится
				IF	(
					op-entry.kau-cr BEGINS "Кредит" 
					AND
					NOT CAN-DO("5,10,36,358,57",ENTRY(3,op-entry.kau-cr))
			 	)
			 	OR
			 	(
			 		op-entry.kau-db BEGINS "Кредит"
...
PUT UNFORMATTED	
	"  6) В Н Е Б А Л А Н С О В Ы Е   О П Е Р А Ц И И" SKIP
	"┌──────┬──────────────────────────────┬───────────┬──────────┬──────┬────────────────────┬────────────────────┬──────────────────┬───┬──────────────────────────────┬───────────┬─────────┬─────────────┐" SKIP.
