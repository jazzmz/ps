...
/*
   ...
   Parameters: Input: RID -  RecID(op)
      ...
      Used by:
      Created: 15/03/06 13:04:28
     Modified:
*/
Form "~n@(#) memcvtb5.p 1.0 RGen 15/03/06 RGen 15/03/06 [ AutoReport By R-Gen ]"
...
find first op-entry of op no-lock no-error.
   if avail op-entry and
      ...
      find first op-entry of op where 
                 ...
                 (substr(op-entry.acct-db,1,5) ne "70606" or
                  substr(op-entry.acct-db,1,5) ne "70205") no-lock no-error.
      if avail op-entry then do:     
        ...
        find first wop-entry of op where 
                   ...
                   (substr(wop-entry.acct-cr,1,5) ne "70601" or
                    substr(wop-entry.acct-cr,1,5) ne "70103") 
                    no-lock no-error.
        if avail wop-entry then do:
        ...
        find first xop-entry of op
             where xop-entry.acct-db begins "70606" or
                   xop-entry.acct-db begins "70205" or
                   xop-entry.acct-cr begins "70601" or
                   xop-entry.acct-cr begins "70103" no-lock no-error.  
           if avail xop-entry then do:
              if xop-entry.acct-db begins "70606" or
                 xop-entry.acct-cr begins "70205" then do:
