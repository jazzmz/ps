...
FUNCTION LnGetProvAcct RETURNS DECIMAL (INPUT iAcct AS CHAR,
   ...
   DEFINE VARIABLE vQualityCategory AS CHARACTER NO-UNDO. /* категория качества обеспечения */
   ...
   NO-LOCK, 
      ...
      NO-LOCK:
         vQualityCategory = Get_QualityGar ("comm-rate",
                                            ...
                                            + STRING(term-obl.nn),
                                            iDate).
         /* если категория качества для обеспечения не определена, то не обрабатываем обеспечение */
