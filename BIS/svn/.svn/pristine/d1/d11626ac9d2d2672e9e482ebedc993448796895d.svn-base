...
DEF BUFFER t2loan-cond for tloan-cond. 
...
DEF VAR vGreisPer AS INT   NO-UNDO.
...
DO ON ERROR UNDO,LEAVE ON ENDKEY UNDO,LEAVE :
   ...
   .
      ...
      IF vFicsDate = "Да" THEN DO:
          ...
          END.           
          ELSE 
              vDate  = tloan.open-date + DEC(GetXAttrValueEx ("loan",
                       loan.contract + "," + loan.cont-code,
                       "ОврПр",
                       GetXAttrInit(loan.class-code,"ОврПр"))). /* если что-то не сложилось, то дату окончания определяем по периоду */
      ...
      DO:
         vDate  = tloan.open-date + DEC(GetXAttrValueEx ("loan",
                       loan.contract + "," + loan.cont-code,
                       "ОврПр",
                       GetXAttrInit(loan.class-code,"ОврПр"))).
         /*message vDate view-as alert-box.*/
...
/*вот тут нужно создать ещё одно условие. первое условие создается как условие для грейса, но на весь срок, второе условие будет менять ставку.
начало условия loan.open-date + 101 день.*/
/*ищем условие на охвате*/

      find last bloan-cond where bloan-cond.contract  = tloan.contract
                             and bloan-cond.cont-code = ENTRY(1,tloan.cont-code," ")
                             and bloan-cond.since <= tloan.open-date 
                             NO-LOCK NO-ERROR.
      if available bloan-cond then 
        DO:
         tloan-cond.int-period = bloan-cond.int-period.
         tloan-cond.cred-period = bloan-cond.cred-period.
       END.

       FIND LAST  comm-rate WHERE
                  comm-rate.commi    EQ "%Грейс"
              AND comm-rate.kau      EQ tloan.contract + "," + ENTRY(1,tloan.cont-code," ")
              AND comm-rate.currency EQ tloan.currency
              AND comm-rate.acct     EQ "0"
              AND comm-rate.since    LE tloan-cond.since  NO-LOCK NO-ERROR.
       ...
       /*определяем срок грейса, это будет значение доп.река на договоре, или реквизита на классе охватывающего договора*/

       vGreisPer = INT(GetXAttrValueEx ("loan",
                       tloan.contract + "," + ENTRY(1,tloan.cont-code," "),
                       "PirGracePer",
                       GetXAttrInit(loan.class-code,"PirGracePer"))).
      
       
       CREATE t2loan-cond .
       BUFFER-COPY tloan-cond EXCEPT since TO t2loan-cond.
       ASSIGN t2loan-cond.since = tloan.open-date + vGreisPer + 1
              t2loan-cond.rcommi = comm-rate.rate-comm.

       FIND LAST  comm-rate WHERE
                  comm-rate.commi    EQ "%Кред"
              AND comm-rate.kau      EQ tloan.contract + "," + ENTRY(1,tloan.cont-code," ")
              AND comm-rate.currency EQ tloan.currency
              AND comm-rate.acct     EQ "0"
              AND comm-rate.since    LE tloan-cond.since  NO-LOCK NO-ERROR.
       ...
       CREATE t2loan-cond .
       BUFFER-COPY tloan-cond EXCEPT since TO t2loan-cond.
       ASSIGN t2loan-cond.since = tloan.open-date + vGreisPer + 33 + 1
              t2loan-cond.rcommi = comm-rate.rate-comm.


   RUN cr_multi_loan-cond_with_tab (tloan.Contract + "," + main-cont-code, OUTPUT mCondRid, OUTPUT mFlagError).
   /*меняем параметр paytype на последнем условии*/

   UpdateSigns("loan-cond",tloan.contract + "," + tloan.cont-code + "," + STRING(tloan.open-date + vGreisPer + 1,"99/99/99"),"PayType","Остаток",YES).

   UpdateSigns("loan-cond",tloan.contract + "," + tloan.cont-code + "," + STRING(tloan.open-date,"99/99/99"),"Грейс","Да",YES). /*Ставим отметку о том что это условие у нас ГРЕЙС*/
