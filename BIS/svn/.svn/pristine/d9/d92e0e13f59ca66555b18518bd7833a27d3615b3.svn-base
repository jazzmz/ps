...
/**
		Распоряжение на безакцептное списание суммы в счет погашения просроченной задолженности.
		Бурягин Е.П.,09.04.2007 10:04
...
FOR FIRST tmprecid NO-LOCK,
    ...
    :
		...
		to_acct = GetLoanAcct_ULL(loan.contract, loan.cont-code, "Кредит", orderDate, false).
		...
		str[1] = "#tabСписать в безакцептном порядке со счета №" + from_acct + " с последующим зачислением на счет №"
					...
					+ "задолженности по кредиту по Кредитному договору №" + loan_info + ", заключенному между "
					+ bankname + " и " + client + ".".
