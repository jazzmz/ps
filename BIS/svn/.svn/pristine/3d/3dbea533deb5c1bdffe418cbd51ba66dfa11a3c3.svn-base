...
FOR EACH tmprecid,
   FIRST op WHERE RECID(op) EQ tmprecid.id NO-LOCK:
	   ...
	   IF NOT AVAILABLE (loan-int) then MESSAGE "Проводка на привязанна к договору!!!" VIEW-AS ALERT-BOX.           
           ...
           do:         
              FIND FIRST person where person.person-id = loan.cust-id NO-LOCK NO-ERROR.
               ...
               if not available(loan-int) then message "Неправильная привязка к договору" view-as alert-box.
...
/*  	      message loan.cont-code op.op VIEW-AS ALERT-BOX.*/
	   ...
	   /****************************************
	    * В случае если договор оканчивается,  *
            * раньше окончания месяца, то считаем  *
            * до даты окончания договора.          *
            ****************************************
            * Автор: Маслов Д. А. Maslov D. A.     *
            * Заявка: #761                         *
            * Дата создания: 23.09.11 15:02        *
            *****************************************/
...
/*                           message idate VIEW-As ALERT-BOX.*/
	  ...
	  /*******************
           ...
           * По заявке: #709
           * Изменение формата после запятой должно быть два знака.
	   * Например: 192.1 должно отображаться как 192.10
           *******************/
