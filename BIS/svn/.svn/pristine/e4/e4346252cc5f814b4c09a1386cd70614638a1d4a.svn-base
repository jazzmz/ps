...
UPDATE  raspDate evidence WITH FRAME frmTmp CENTERED SIDE-LABELS OVERLAY.
oTpl = new TTpl("pirloan547.tpl").
...
FOR EACH tmprecid,
   FIRST loan WHERE RECID(loan) EQ tmprecid.id NO-LOCK:
         do:
           ...
           find last loan-acct where loan-acct.cont-code = loan.cont-code and (loan-acct.acct-type eq "КредРасч") and loan-acct.since <= end-date  NO-LOCK NO-ERROR.
           ...
           find last loan-acct where loan-acct.cont-code = loan.cont-code and (loan-acct.acct-type eq "КредШт2%") and loan-acct.since <= end-date NO-LOCK NO-ERROR.
	   ...
	   find last loan-acct where loan-acct.cont-code = loan.cont-code and (loan-acct.acct-type eq "КредН" OR loan-acct.acct-type eq "КредЛин") NO-LOCK NO-ERROR.
...
/*                           message idate VIEW-As ALERT-BOX.*/
          ...
          rPog = 0.
          RUN STNDRT_PARAM(loan.contract, loan.cont-code,  92, dperiodend, OUTPUT rPog , OUTPUT dT1, OUTPUT dT1).
