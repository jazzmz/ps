...
DEFINE VARIABLE cKlPoleL  AS CHARACTER EXTENT 9
   /* ФЛ */ INIT ["5,18,19,24,25,27,37,38,41,42,44,46,47,48,49,50,60,63,61,62,64,65,66",
   /* ИП */       "2,12,14,15,16,17,19,20,21,22,26,30,35,36,37,38,40,42,43,45,47,48,49,50,60,61,63,62,64,65,66",
   ...
   /* ИП ВП */    "2,12,14,15,16,17,19,20,21,22,26,30,40,42,43,45,47,65,66,74",
   /* ЮЛ ВП */    "1,9,11,12,14,15,16,17,19,21,22,26,29,30,31,33,34,65,66,74",
...
*/
	...
	END.
      ...
      END.
      WHEN "48" THEN DO:
         cTmpS  = GetXAttrValue(cKlTabl, cKlNum, "Статус_ИПДЛ").
         cKlP   = IF (cTmpS EQ "") THEN "НЕТ" ELSE "Д А".
      END.
      WHEN "49" THEN DO:
         cTmpS  = GetXAttrValue(cKlTabl, cKlNum, "ОтнОкруж_ИПДЛ").
         cTmpS2 = GetXAttrValue(cKlTabl, cKlNum, "СтепРодст_ИПДЛ").
         cKlP   = IF (cTmpS + cTmpS2 EQ "") THEN "НЕТ" ELSE "Д А".
      END.
      WHEN "50" THEN DO: /* В списках cKlPoleL должна следовать сразу за 49, берет оттуда cTmpS и cTmpS2 */
         cKlP   = "".
         IF (cTmpS NE "")
         THEN DO:
            iTmpI = NUM-ENTRIES(cTmpS, ";").
            cKlP  = cKlP
                  + "|      Фамилия, имя и (при наличии) отчество: "
                  + PrintStringInfo(ENTRY(1, cTmpS, ";"))
                  + "|      Занимаемая должность: "
                  + PrintStringInfo(ENTRY(2, cTmpS, ";"))
                  + "|      Наименование Государства: "
                  + PrintStringInfo(ENTRY(3, cTmpS, ";"))
                  + "|      Степень родства или иное отношение: "
                  + PrintStringInfo(ENTRY(4, cTmpS, ";")).
         END.

         IF (cTmpS2 NE "")
         THEN DO:
            iTmpI = NUM-ENTRIES(cTmpS2, ";").
            cKlP  = cKlP
                  + "|      Фамилия, имя и (при наличии) отчество: "
                  + PrintStringInfo(ENTRY(1, cTmpS2, ";"))
                  + "|      Занимаемая должность: "
                  + PrintStringInfo(ENTRY(2, cTmpS2, ";"))
                  + "|      Наименование Государства: "
                  + PrintStringInfo(ENTRY(3, cTmpS2, ";"))
                  + "|      Степень родства или иное отношение: "
                  + PrintStringInfo(ENTRY(4, cTmpS2, ";")).
         END.
      END.
      WHEN "51" THEN DO:
