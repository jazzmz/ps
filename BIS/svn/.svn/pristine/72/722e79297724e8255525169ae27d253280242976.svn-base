...
DEF VAR cVariants AS CHAR INIT "810,840,978" NO-UNDO.

RUN Fill-SysMes IN h_tmess ("","",3,"Выберите валюту|" + cVariants).
...
for each bfloan-int where bfloan-int.contract = "Кредит"
    ...
    for each loan-int where loan-int.contract = "Кредит"
		        ...
		        and ((loan-int.id-k = 0 and loan-int.id-d = 7) /*срочную задолжность в просрочку операц 2*/
