/*
		Выдача кредитов.
		Первый раздел отчета по операциям по кредитным договорам.
		Бурягин Е.П., 16.01.2006 9:38
*/
...
/* Хотя это и первый раздел, но все же. 
   Сперва очистим таблицу
*/
FOR EACH replines
:
	DELETE replines.
...
FOR EACH tmprecid 
   ...
   FIRST op-entry OF op WHERE
         NO-LOCK
: 
	/* Операция по выдаче кредита - это операция под номером 4.
	   Она должна быть прописана в поле kau-db (субаналитика по дебету) проводки.
	   Формат записи в этом поле такой: "Кредит,<НОМЕР_ДОГОВОРА>,<НОМЕР_ОПЕРАЦИИ>".
	   
	   НО 
	   по кредиту субаналитическая проводка не должна быть операцией с кодом 5
	*/
	IF op-entry.kau-db <> "" THEN
		IF NUM-ENTRIES(op-entry.kau-db) = 3 THEN
			IF ENTRY(1,op-entry.kau-db) = "Кредит" AND ENTRY(3, op-entry.kau-db) = "4" 
			  AND (
			  	  op-entry.kau-cr = "" OR (
			  		NUM-ENTRIES(op-entry.kau-cr)=3 
			  		AND 
			  		ENTRY(3,op-entry.kau-cr)<>"5"
			  	)
				) 
			THEN
				DO:
...
PUT UNFORMATTED	
	"  1) В Ы Д А Ч А   К Р Е Д И Т О В" SKIP
	"┌──────┬──────────────────────────────┬───────────┬──────────┬──────┬────────────────────┬────────────────────┬──────────────────┬───┬──────────────────────────────┬───────────┬─────────┬─────────────┐" SKIP.
...
PUT UNFORMATTED	
	"│      │                              │           │          │      │                    │      ИТОГО(выдано):│" subtotal_amt_rub FORMAT ">>>,>>>,>>>,>>9.99" 
																																																																   "│810│                              │           │         │             │" SKIP 
