...
/** 
		...
		Экспорт данных в формате afx для кредитных договоров и дополнительных соглашений
		для  Индивидуальных Предпринимателей
		
		Ситов С.А., 13.11.2010
		...
		<Особенности_реализации> Процедура создана на основе pirexocredfizlim.p 
					ПОЭТОМУ есть неиспользуемые поля, но они пригодятся при доработке
...
FUNCTION exp_client RETURNS CHAR (
			...
			construction = construction + "svidregist=" + arg2 + CHR(13) + CHR(10).
			construction = construction + "inn=" + arg3 + CHR(13) + CHR(10).
			construction = construction + "ogrn=" + arg4 + CHR(13) + CHR(10).
			construction = construction + "addressur=" + arg5 + CHR(13) + CHR(10).
			construction = construction + "acct=" + arg6 + CHR(13) + CHR(10).
			construction = construction + "predstcorp=" + arg7 + CHR(13) + CHR(10).
			construction = construction + "predstdover=" + arg8 + CHR(13) + CHR(10).
			...
			IF AVAIL cust-corp THEN
				DO:
					PUT UNFORMATTED StrToWin_ULL(exp_client(
							(
							if cust-corp.cust-stat = "ИП" then replace(cust-corp.name-corp,chr(34),"``") 
							else  replace(GetXAttrValueEx("cust-corp",string(cust-corp.cust-id),"FullName",""),chr(34),"``")
							), 
							...
							account ,
							"" ,		/*Представитель ЮрЛ*/                 
							"" 		/*на основании доверенности (устава)*/
						)
					).
				END.
		END.
