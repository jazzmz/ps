...
/*
 ...
 27/03/06  - Отчет о счетах по которым нет оборотов за период.
...
def input parameter bal-line as char format "x(3000)".
...
put unformatted "-------------------- --------- ---------- -------------------------------" skip.
put unformatted	"Лицевых счетов без движения за период c " beg-date " по " end-date skip.
put unformatted	"открытых на счетах 2-го порядка:" bal[1] skip. 
    DO WHILE (bal[i] NE ""):
        PUT UNFORMATTED "                                " string(bal[i],"x(60)") skip.
       i = i + 1.
    END.
