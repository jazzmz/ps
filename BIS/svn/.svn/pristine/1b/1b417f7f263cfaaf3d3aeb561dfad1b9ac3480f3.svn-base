{pirsavelog.p}
	...
	/* Sitov S.A. 2011_11_30 */
	/* Сделано на основе pirexpcard.p */
	/* ЭТО ВРЕМЕННАЯ ПРОЦЕДУРА ДЛЯ КОРЯВЫХ ДОГОВОРОВ ОТКРЫТЫХ 06/05/2005*/
...
: 
	...
	IF account = "" THEN
		...
		FIND FIRST acct WHERE acct.acct = account AND acct.open-date = 05/06/05 NO-LOCK NO-ERROR.
		IF avail (acct) THEN 
			account =  acct.acct.
		ELSE 
			account = "НА дату 06/05/05 нет счета!!!" .
		END.

	IF account_usd = "" THEN	
		DO:
		account_usd = GetLoanAcct_ULL_sur(loan.contract, loan.cont-code, 'SCS@840').
		FIND FIRST acct WHERE acct.acct = account_usd AND acct.open-date = 05/06/05 NO-LOCK NO-ERROR.
		IF avail (acct) THEN 
			account_usd =  acct.acct.
		ELSE 
			account_usd = "НА дату 06/05/05 нет счета!!!" .
		END.

	IF account_eur = "" THEN
		DO:
		account_eur = GetLoanAcct_ULL_sur(loan.contract, loan.cont-code, 'SCS@978').
		FIND FIRST acct WHERE acct.acct = account_eur AND acct.open-date = 05/06/05 NO-LOCK NO-ERROR.
		IF avail (acct) THEN 
			account_eur =  acct.acct.
		ELSE 
			account_eur = "НА дату 06/05/05 нет счета!!!" .
		END.
