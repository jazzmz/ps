...
/*FOR EACH Dataline OF DataBlock where Dataline.Sym2 = "Общие данные" NO-LOCK:*/
        ...
        oTpl = new TTpl("pir_321_soop.tpl").
        oTable = new TTable(3).
	...
	oTable:addRow().
	oTable:addCell("Сведения об операции (сделке)").

	oTable:addRow().
	oTable:addCell("Вид операции (нужное поменить 'Х')").

	oTable:addRow().
	oTable:addCell("- oперация, подлежащая обязательному контролю").
	if VO ne "6001" then oTable:addCell("x").
	else oTable:addCell("").
	
	oTable:addRow().
	oTable:addCell("- операция, в отношении которой возникают подозрения, что она осуществляется в целях легализации (отмывания) доходов, полученных преступным путем, или финансирования терроризма").
	if VO eq "6001" then oTable:addCell("x").
	else oTable:addCell("").

	oTable:addRow().
	oTable:addCell("Содержание операции (сделки) ").
	oTable:addCell(ENTRY(11, DataLine.Txt, "~n") + if ENTRY(12, DataLine.Txt, "~n") ne "0" then ENTRY(12, DataLine.Txt, "~n") else "").

	oTable:addRow().
	oTable:addCell("Дата операции (сделки)").
	oTable:addCell(string(date(ENTRY(9, DataLine.Txt, "~n")),"99/99/9999")).

	oTable:addRow().
	oTable:addCell("Валюта операции (сделки)").
	oTable:addCell(ENTRY(10, DataLine.Txt, "~n") + IF ENTRY(10, DataLine.Txt, "~n") EQ "643" THEN "" ELSE " / 643").

	oTable:addRow().
	oTable:addCell("Сумма операции (сделки)").
	...
	oTable:addRow().
	oTable:addCell("Сведения о лице (лицах), участвующем (участвующих) в операции (сделке)").
	...
	if avail DatalinePL then do:	
		oTable:addRow().
		oTable:addCell("Плательщик (лицо, совершающее операцию/сделку)").
		oTable:addCell(ENTRY(3, DataLinePL.Txt, "~n")).

		oTable:addRow().
		oTable:addCell("Кредитная организация плательщика").
		oTable:addCell(ENTRY(45, DataLinePL.Txt, "~n") + ", " + ENTRY(43, DataLinePL.Txt, "~n")).

		oTable:addRow().
		oTable:addCell("Счет плательщика").
		oTable:addCell(ENTRY(38, DataLinePL.Txt, "~n")).
	end.
	else do:
		oTable:addRow().
		oTable:addCell("Плательщик (лицо, совершающее операцию/сделку)").
	...
	FIND FIRST DatalinePo where DatalinePo.Data-ID EQ ipDataID and DatalinePo.Sym2 eq "Получатель" and DatalinePo.Sym1 eq Dataline.Sym1 
		...
		oTable:addRow().
		oTable:addCell("Кредитная организация получателя").
		oTable:addCell(ENTRY(45, DataLinePo.Txt, "~n")).

		oTable:addRow().
		oTable:addCell("Счет получателя").
		oTable:addCell(ENTRY(38, DataLinePo.Txt, "~n")).
        end.
	else do:
		oTable:addRow().
		oTable:addCell("Плательщик (лицо, совершающее операцию/сделку)").
	...
	oTable:addRow().
	oTable:addCell("Сведения о сотруднике, составившем сообщение").

	if mW ne "пусто" then do:
        /*	RUN browseld.p ("_user","_Userid","0" + string(mW) + "*", ?, 1).*/
        	{ttretval.def}
                RUN browseld.p ("_user","_userid" + CHR(1) + "RetRcp"+ CHR(1) + "RetFld",
        			"*" + mW + "*" + CHR(1) + STRING(TEMP-TABLE ttRetVal:HANDLE) + CHR(1) + "_user._userid", ?, 3).

        	find first _user where _user._userid eq string(ttRetVal.PickValue) NO-LOCK no-error.
	end.
	if avail _user then do:
		oTable:addRow().
		oTable:addCell("Подразделение Банка").
		oTable:addCell(GetXAttrValueEx("_user", _user._userid, "group-id", "")).

		oTable:addRow().
		oTable:addCell("Ф.И.О").
		oTable:addCell(_user._User-Name).

		oTable:addRow().
		oTable:addCell("Должность (поля, добавленные Банком)").  
	...
	oTable:addRow().
	oTable:addCell("Подпись").  
	oTable:addCell("").

	DEF VAR date_s AS char format "99/99/9999" LABEL "Дата составления" VIEW-AS FILL-IN NO-UNDO.
	DEF VAR time_s AS char format "99:99" LABEL "Время составления" VIEW-AS FILL-IN NO-UNDO.
	SET date_s time_s WITH FRAME frmTmp4 OVERLAY CENTERED.
        HIDE FRAME frmTmp4.

	oTable:addRow().
	oTable:addCell("Дата и время составления сообщения").  
	oTable:addCell(string(date_s,"99/99/9999") + " " + string(time_s,"99:99")).

	oTable:addRow().
	oTable:addCell("Подпись руководителя подразделения").  
	oTable:addCell("").

	DEF VAR date_p AS char format "99/99/9999" LABEL "Дата получения" VIEW-AS FILL-IN NO-UNDO.
	DEF VAR time_p AS char format "99:99" LABEL "Время получения" VIEW-AS FILL-IN NO-UNDO.
	SET date_p time_p WITH FRAME frmTmp3 OVERLAY CENTERED.
        HIDE FRAME frmTmp3.

	oTable:addRow().
	oTable:addCell("Отметка сотрудника Подразделения ПОД/ФТ о получении Сообщения").  

	oTable:addRow().
	oTable:addCell("Дата и время получения").  
	...
	oTable:addRow().
	oTable:addCell("Ф.И.О. и подпись").  
	oTable:addCell(_user._User-Name).
...
*/
		...
		if VO ne "6001" and VO ne "0" then do:
			...
			if ENTRY(8, DataLine.Txt, "~n") ne "0" and ENTRY(8, DataLine.Txt, "~n") ne VO 
					and not Can-do("*6001*",ENTRY(8, DataLine.Txt, "~n"))	 
			...
			if ENTRY(8, DataLine.Txt, "~n") ne "0" and ENTRY(8, DataLine.Txt, "~n") ne VO 
					and Can-do("*6001*",ENTRY(8, DataLine.Txt, "~n"))	 
					and NUM-ENTRIES(ENTRY(8, DataLine.Txt, "~n")) > 1
        				and ENTRY(16, DataLine.Txt, "~n") ne "0" then do:
        			oTable:addRow().
        			oTable:addCell("Принято решение").  
				if INDEX(ENTRY(8, DataLine.Txt, "~n"),"6001") > 1 then
					DopVO = SUBSTRING(ENTRY(8, DataLine.Txt, "~n"),1,INDEX(ENTRY(8, DataLine.Txt, "~n"),"6001") - 1).
				if INDEX(ENTRY(8, DataLine.Txt, "~n"),"6001") + 5 < LENGTH(ENTRY(8, DataLine.Txt, "~n")) then
					DopVO = DopVO + SUBSTRING(ENTRY(8, DataLine.Txt, "~n"),INDEX(ENTRY(8, DataLine.Txt, "~n"),"6001") + 5). 
				if SUBSTRING(DopVO,LENGTH(DopVO),1) eq "," then
					DopVO = SUBSTRING(DopVO,1,LENGTH(DopVO) - 1).
				oTable:addCell("Представить сведения в УО об операции/сделке обязательного контроля в соотв. со ст.6 Закона 115-ФЗ по кодам " 
						+ VO + "," + DopVO +
						". Представить сведения в УО о подозрительной операции/сделке в соотв. с п.3 ст.7 Закона 115-ФЗ по коду 6001, признак кода "
						+ ENTRY(16, DataLine.Txt, "~n")).
