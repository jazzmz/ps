...
DEFINE VARIABLE outStrArray3 AS CHARACTER EXTENT 6 NO-UNDO.     /* Определение массива для хранения строки таблицы */
	...
	FIRST loan WHERE tmprecid.id = RECID(loan) NO-LOCK.
		DO:
			   ...
			   RUN acct-pos in h_base (loan-acct.acct, loan-acct.currency,gend-date,gend-date, chr(251)).
				...
				WHEN "КредПр%" THEN
					DO:
					  outStrArray3[1] = loan-acct.acct.
					  outStrArray3[2] = cAcctBal.
					   dCol2Summ = ROUND(dCol2Summ + DECIMAL(cAcctBal),iPrecession).
					END.

				WHEN "КредРезПр" THEN
					DO:
					  outStrArray3[3] = loan-acct.acct.
					  outStrArray3[4] = cAcctBal.
					   dCol4Summ = ROUND(dCol4Summ + DECIMAL(cAcctBal),iPrecession).
					END.	
		         ...
		         outStrArray3[5] = STRING( ROUND(100 / DECIMAL(outStrArray3[2]) * DECIMAL(outStrArray3[4]),iPrecession) ) NO-ERROR.
			      ...
			      IF isValidate(outStrArray3[1],outStrArray3[2],outStrArray3[3],outStrArray3[4]) THEN
		                       RUN put2Report(outStrArray3[1],outStrArray3[2],outStrArray3[3],outStrArray3[4],outStrArray3[5],outStrArray3[6]).
