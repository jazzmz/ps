...
DEFINE VARIABLE mW AS INTEGER NO-UNDO INITIAL 0
    VIEW-AS RADIO-SET RADIO-BUTTONS " Погашение просроченных %",2,
				" Погашение %",3,
...
for each loan-int where loan-int.contract = "Кредит"
		        and ((loan-int.id-d = 30 and loan-int.id-k = 48 and mW = 2) /*погашение просроченных % */
		        or  (loan-int.id-d = 6 and loan-int.id-k = 4 and mW = 3) /*погашение % */
		        or  (loan-int.id-d = 30 and loan-int.id-k = 29 and mW = 4)) /*погашение начисленных % */
    ...
    if available loan then do:
		    find first op-entry where op-entry.op = loan-int.op and can-do(cur,op-entry.currency) NO-LOCK NO-ERROR.
			    ...
			    if loan-int.id-k = 48 then 
			    oTable:AddCell("Погашение просроченных процентов ").
		
			    /*погашение % */
			    if loan-int.id-k = 6 then do: 
			    ...
			    if loan-int.id-k = 35 then do:
				    /* находим было ли нач. процентов. это будет дата ПО */
				    find last bloan-int where bloan-int.contract = loan.contract
	   			        and bloan-int.cont-code = loan.cont-code 
			        	and bloan-int.id-k = 32 and bloan-int.id-d = 33 /* требование по нач. % */
			    ...
			    if loan-int.id-k = 26 then do:
					...
					oTable:AddCell("Оплата неустойки за просрочку основного долга с " + STRING(bloan-int.mdate + 1) + " по " + STRING(end-date)).
			 	  	...
			 	  	oTable:AddCell("Оплата неустойки за просрочку основного долга с " + STRING(loan.open-date + 1) + " по " + STRING(end-date)).
  		    ...
  		    else do: message "не нашел документ". pause. end.
