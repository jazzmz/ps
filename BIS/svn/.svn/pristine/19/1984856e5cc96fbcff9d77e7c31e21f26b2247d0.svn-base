...
FOR EACH all_recids, 
  ...
  bDel = true.
     ...
     first loan-int where loan-int.contract   = bloan.contract
                         ...
                         NO-LOCK.
            bDel = false.
      message  bDel VIEW-AS ALERT-BOX.
      end.
...
/*        message "Повторный запуск по договору " loan.cont-code VIEW-AS ALERT-BOX.*/
  ...
  if bDel = false THEN  MESSAGE "Закрыть лимит по договору " + loan.cont-code + "?"  VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE BtOk. 
