/*переделанный отчет pir_rsrv283.p для работы с проводками по резерву по договору Баймагамбетова.
немного скореектирован формат вывода.
...
oTpl = new TTpl("pir_rsrv283bai.tpl").
...
/*RUN ReqAcctByRole in h_loan ("Кредит",
   ...
   find last loan-acct where loan-acct.contract = "КРедит" and loan-acct.acct-type = "КредБудПени" and loan-acct.cont-code = loannum NO-LOCK NO-ERROR.

   if available loan-acct then acct_main_no = loan-acct.acct.
   else MESSAGE "Ошибка поиска счета" VIEW-AS ALERT-BOX.
   ...
   acct_main_pos_rur = ROUND(acct_main_pos_rur,2).
