...
/* ------------------------------------------------------
     ...
     Базируется: 
     Причина: 
     Что делает: Формирует печатную форму распоряжения по изменению процентной ставки.
     Как работает: 
     Параметры: 
...
DEF VAR newRate AS DECIMAL LABEL "Ставка %" FORMAT ">9.99999" NO-UNDO.
DEF VAR newRateStr AS CHAR EXTENT 2 NO-UNDO.
DEF VAR newRateDate AS DATE LABEL "С даты" FORMAT "99/99/9999" NO-UNDO.
...
FOR FIRST tmprecid NO-LOCK,
	...
	:
		...
		str[1] = "#tabУстановить с " + STRING(newRateDate, "99.99.9999") + " процентную ставку за пользование кредитом в размере "
		         + STRING(newRate) + "% (" + newRateStr[1] + "целых " + newRateStr[2]  
		         + "сотых процентов) годовых по Кредитному договору №" + loaninfo
		         + ", заключенному между " + client + " и " + bankname + "."
