...
/* ------------------------------------------------------
File          : $RCSfile: pirop-enbum.p,v $ $Revision: 1.2 $ $Date: 2007-10-18 07:42:23 $
Copyright     : ООО КБ "Пpоминвестрасчет"
Базируется    : op-enbum.p
Причина       : Приказ №64 от 25.10.2005
Параметры     : Строка с именем файла:паратмеры для op-enbum.p 
Место запуска : БМ/Печать/Выходные формы/План счетов (приложения )/Приложение 10/Журналы по отделениям
Автор         : ???? 
Изменения     : $Log: not supported by cvs2svn $
Изменения     : Revision 1.1  2007/08/16 11:08:20  Lavrinenko
Изменения     : Переделано определение имени выходго файла - передается черезпараметрт
Изменения     :
------------------------------------------------------ */
{op-enbum.def}
{intrface.get xclass}
DEF INPUT PARAM iRaram AS CHAR NO-UNDO. /* имя фала + код подразделения + код отделения */

DEFINE VARIABLE cldate_type AS CHAR NO-UNDO. /* код подразделения + код отделения */
...
{getdate.i}
{get-bankname.i}
{pirraproc.i &in-end-date=end-date &arch_file_name="ENTRY(1,iRaram,':')" &arch_file_name_var=yes)}
...
{setdest.i &cols=130 &filename=arch_file_name}
