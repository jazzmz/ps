...
DEFINE VARIABLE cCountry     AS CHAR NO-UNDO.
...
/******************************************* Присвоение значений переменным и др. */
    ...
    ttBusinessmen.visaSeries = ttBusinessmen.migrationCard.
    ...
    cCountry = GetXAttrValue("cust-corp", STRING(cust-corp.cust-id), "country-id2").
    FIND FIRST country
       WHERE country.country-id EQ cCountry
       NO-LOCK NO-ERROR.
...
*/
    ...
    NO-ERROR.
    ttBusinessmen.addressOfLaw = IF AVAIL(cust-ident) THEN cust-ident.issue ELSE ?.
    ...
    FIND FIRST cust-ident WHERE
         ...
         AND cust-ident.close-date     EQ ?
    NO-ERROR.
    ttBusinessmen.addressOfStay = IF AVAIL(cust-ident) THEN cust-ident.issue ELSE ?.
    ...
    FIND FIRST cust-ident WHERE
         ...
         AND cust-ident.close-date     EQ ?
    NO-ERROR.
    ttBusinessmen.addressOfPost = IF AVAIL(cust-ident) THEN cust-ident.issue ELSE ?.
