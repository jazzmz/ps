...
/** 
   ОТЧЕТ.
   ООО КБ "ПРОМИНВЕСТРАСЧЕТ", Управление автоматизации (с) 2009-2013
...
Modified: 30/08/11 SStepanov для представителя Банка - сокр.наименование из ДР, ИНН из cust-ident по запросу Капитановой
	  ...
	  01/03/13..08/04/13 AGoncharov доработки по новым правила ПОДФТ (в частности, пункт #33.1)
	  10/04/13 AGoncharov Анкета представителя ИП (новинка!11)
	  11/04/13 AGoncharov полная ревизия всех веток первого CASE (Ч, Ю, Б, ВП, П), ибо сам там запутался. Зато, сделал красиво и более-менее понятно. :)
	  30/04/13 Agoncharov завершенно тестирование и исправление недочётов по тексту
*/
...
DEFINE VARIABLE cKlPoleL  AS CHARACTER EXTENT 10
   /* ФЛ */ INIT ["5,18,19,24,25,27,37,38,41,42,44,46,47,48,49,60,61,62,63,64,65,66,77,78,90",
   /* ИП */       "2,12,14,15,16,17,19,20,21,22,26,30,35,36,37,38,40,42,43,45,47,48,49,60,61,62,63,64,65,66,77,78,90",
   /* ЮЛ */       "1,9,11,12,14,15,16,17,19,21,22,26,29,30,31,33,33.1,34,35,36,37,38,60,61,62,63,64,65,66",
   /*  Б */       "8,10,11,13,14,15,16,17,19,21,23,28,29,30,32,33,33.1,34,37,38,51,60,61,62,63,64,65,66",
   /* ФЛ ВП */    "5,18,19,24,25,27,33,33.1,41,42,44,46,47,61,62,65,66,74,79",
   /* ИП ВП */    "2,12,14,15,16,17,19,20,21,22,26,30,40,42,43,45,47,61,62,65,66,74,79",
   /* ЮЛ ВП */    "1,9,11,12,14,15,16,17,19,21,22,26,29,30,31,33,33.1,34,61,62,65,66,74,79",
   /* ФЛ П  */    "70,71,72,73,74,75,76,5,18,19,24,25,27,33,33.1,41,42,44,46,47,61,62,65,66",
   /* ЮЛ П  */    "70,71,72,73,74,75,76,1,9,11,12,14,15,16,17,19,21,22,26,29,30,31,33,33.1,61,62,65,66",
   /* ИП П  */    "2,17,19,12,14,15,16,20,21,22,26,30,40,42,43,45,47,61,62,65,66,70,71,72,73,74,75,76"]              NO-UNDO.

   /* ИП П      "2,40,42,43,70,71,72,73,74,75,76,5,18,19,24,25,27,33,33.1,41,44,46,47,61,62,65,66"]              NO-UNDO. */
...
*/
      ...
      END.
      WHEN "33" THEN DO:
         cTmpS  = ENTRY(1, GetXAttrValue(cKlTabl, cKlNum, "УставКап"),"/").
	 cKlP   = PrintStringInfo(cTmpS).
			
      END.
      WHEN "33.1" THEN DO:
		if can-do ("*/*", GetXAttrValue(cKlTabl, cKlNum, "УставКап")) then
                	cTmpS  = ENTRY(2, GetXAttrValue(cKlTabl, cKlNum, "УставКап"),"/").
		else cTmpS = "".
		cKlP   = PrintStringInfo(cTmpS).	
      END.
      WHEN "34" THEN DO:
