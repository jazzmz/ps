...
oTpl = new TTpl("pir_rsrv_39_.tpl").
                      ...
                      IF AVAIL loan THEN 
                            DO:                                       
                                        ...
                                        IF AVAILABLE(bOpEntry) THEN dbeg = bOpEntry.op-date + 1.
                                            ...
                                            oTable:addCell(loan.doc-num + "    " + STRING(bTerm-Obl.amt-rub) + " RUB").
                                            oTable:addCell(dbeg).
                                            oTable:addCell(dend).
                                            ...
                                            oTable:addCell("810").
                                            oTable:addCell(STRING(GetLoanCommissionEx_ULL(loan.contract,loan.cont-code,"%Рез",op.op-date,false,comType)) + comType).
                                            oTable:addCell(op-entry.acct-db).
                                            oTable:addCell(op-entry.acct-cr).
/*                                            dMbeg = MIN(Date(dMbeg),Date(dbeg)).
                                            dMend = MAX(Date(dMend),Date(dend)).*/
...
oTpl:addAnchorValue("НАЧАЛОПЕРИОДА",dbeg).
oTpl:addAnchorValue("КОНЕЦПЕРИОДА",dend).
