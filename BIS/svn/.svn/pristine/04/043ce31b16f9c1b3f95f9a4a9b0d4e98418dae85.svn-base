...
/*                  op-entry.op-date   >= MAX(beg, loan-acct.since) AND*/
                  op-entry.op-date   >= MAX(DATE(01,01,YEAR(beg)), loan-acct.since) AND
...
/*                  op-entry.op-date   >= MIN(beg, loan-acct.since) AND*/
                  op-entry.op-date   >= MIN(DATE(01,01,YEAR(beg)), loan-acct.since) AND
...
/* Сумма амортизации на начало текущего года*/
   RUN НачАморНал    IN mH (DATE(01, 01, YEAR(dob)) ,
	                    OUTPUT mYearAmor
			    ).
/*  message mLastAmor mYearAmor VIEW-AS ALERT-BOX.*/

  if mLastAmor <> 0 then  
do:
/*if mYearAmor <> mLastAmor then */
/*message /*mLastAmor mYearAmor */ mMonSum[1] mMonSum[2]  loan.cont-code VIEW-AS ALERT-BOX.            */
/*mYearAmor =  mLastAmor - mYearAmor.                                                                */
/*if mYearAmor <> mMonSum[1] then  message mYearAmor mMonSum[1] loan.cont-code VIEW-AS ALERT-BOX.*/
end.  
  mYearAmor = 0.
   DO mCnt = 01 TO MONTH(dob):
      ASSIGN
         mYearAmor = mYearAmor + mMonSum[mCnt].
/*   message mCnt VIEW-AS ALERT-BOX.*/
   END.


/*mYearAmor = /*mInitAmor - mYearAmor + */ mMonSum[1] + mMonSum[2] + mMonSum[3] + mMonSum[4] + mMonSum[5] + mMonSum[6] + mMonSum[7].*/
