...
/* ------------------------------------------------------
File          : $RCSfile: pir_f207_ved2.p,v $ $Revision: 1.6 $ $Date: 2011-01-25 12:50:54 $
Copyright     : ООО КБ "Пpоминвестрасчет"
Назначение    : Ведомость документов отобранных для 207П
Автор         : 13.10.2005      ANISIMOV
Изменения     : $Log: not supported by cvs2svn $
Изменения     : Revision 1.5  2010/12/08 07:40:22  borisov
Изменения     : Zagolovok: 321-P
Изменения     :
...
DEF VAR dbReg     AS CHAR NO-UNDO.
DEF VAR crReg     AS CHAR NO-UNDO.
...
*/
   ...
   mDebName[1] = TRIM(acct.Details).
     ...
     mDebName[1] = NameCli[1] + " " + NameCli[2].
     			...
     			IF dbacc begins "20202" and cracc begins "20202810" then
     			 mDebName[1] = GetXAttrValueEx("op",
                               STRING(op-entry.op),
                               "ФИО",
                               mDebName[1])
                         + " " +
   ...
   dbMov = lastmove.
   CASE acct.cust-cat:
      WHEN "Ю" THEN dbReg = STRING(DATE(GetXAttrValue("cust-corp", STRING(acct.cust-id), "RegDate")), "99.99.9999").
      WHEN "Ч" THEN DO:
            FIND FIRST person
               WHERE (person.person-id EQ acct.cust-id)
               NO-LOCK NO-ERROR.
            dbReg = STRING(person.birthday, "99.99.9999").
         END.
      WHEN "Б" THEN dbReg = STRING(DATE(GetXAttrValue("banks"    , STRING(acct.cust-id), "RegDate")), "99.99.9999").
      WHEN "В" THEN dbReg = "          ".
   END CASE.
   ...
   FIND FIRST acct WHERE acct.acct EQ cracc NO-LOCK NO-ERROR.
    ...
    mCreName[1] = NameCli[1] + " " + NameCli[2].
    			...
    			IF dbacc begins "20202810" and cracc begins "20202" then
     			 mDebName[1] = GetXAttrValueEx("op",
                               STRING(op-entry.op),
                               "ФИО",
                               mDebName[1])
   ...
   crMov = lastmove.
   CASE acct.cust-cat:
      WHEN "Ю" THEN crReg = STRING(DATE(GetXAttrValue("cust-corp", STRING(acct.cust-id), "RegDate")), "99.99.9999").
      WHEN "Ч" THEN DO:
            FIND FIRST person
               WHERE (person.person-id EQ acct.cust-id)
               NO-LOCK NO-ERROR.
            crReg = STRING(person.birthday, "99.99.9999").
         END.
      WHEN "Б" THEN crReg = STRING(DATE(GetXAttrValue("banks"    , STRING(acct.cust-id), "RegDate")), "99.99.9999").
      WHEN "В" THEN crReg = "          ".
   END CASE.
