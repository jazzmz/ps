...
FOR EACH tmprecid, FIRST acct WHERE recid(acct) = tmprecid.id NO-LOCK:
   ...
   for each op-entry where op-entry.acct-db eq acct.acct
      ...
      if op-entry.acct-cr eq ? THEN 
         find first xop-entry of op where xop-entry.acct-db eq ? no-lock no-error.
      ...
      IF FIRST-OF(op.user-inspector) THEN DO:    
         ...
         display caps(br-name) @ name-bank skip(2)                   
                "äÄëëéÇõâ " skip                                    
                "ÜìêçÄã èé èêàïéÑì" skip                            
                 "áÄ"  date-rep "Ñ-í " long-acct SKIP(2)            
      ...
      IF LAST-OF(op.user-id) and suppinsp THEN DO:
         ...
         display "àíéÉé" @ op.doc-num
                 ...
                 ins-count @ acct.acct
                 accum total by op.user-id op-entry.amt-cur @ op-entry.amt-cur
                 accum total by op.user-id op-entry.amt-rub @ op-entry.amt-rub with frame cs.
         DOWN 1.
         doc-count = 0.
         ins-count = 0.
      END.
