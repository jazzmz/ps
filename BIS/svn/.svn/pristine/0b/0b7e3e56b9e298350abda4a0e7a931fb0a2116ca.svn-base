...
DEFINE RECTANGLE RECT-1
     ...
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 50 BY 15
     &ELSE SIZE 50 BY 10.71 &ENDIF.

DEFINE VARIABLE isFast AS LOGICAL INITIAL no 
     LABEL "" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME FRAME-A
     client
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 2 COL 10 COLON-ALIGNED
          &ELSE AT ROW 2 COL 13.57 COLON-ALIGNED &ENDIF NO-LABEL
     country
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 4 COL 10 COLON-ALIGNED
          &ELSE AT ROW 7.85 COL 13.71 COLON-ALIGNED &ENDIF NO-LABEL
     rdCurrency
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 6 COL 12
          &ELSE AT ROW 5 COL 41.57 &ENDIF NO-LABEL
     SUM
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 9 COL 10 COLON-ALIGNED
          &ELSE AT ROW 4 COL 10 COLON-ALIGNED &ENDIF NO-LABEL
     isFast
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 11 COL 12
          &ELSE AT ROW 9.64 COL 10 &ENDIF
     "äãàÖçí:" VIEW-AS TEXT
...
ON "GO" OF FRAME FRAME-A DO:
  out_Result = STRING(currPerson) + "," + client:SCREEN-VALUE + "," + country:SCREEN-VALUE + "," + REPLACE(sum:SCREEN-VALUE,",","") + "," + rdCurrency:SCREEN-VALUE + "," + STRING(INPUT isFast).
END.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY client SUM  country isFast rdCurrency
      WITH FRAME FRAME-A IN WINDOW TERMINAL-SIMULATION.
  ENABLE RECT-1 client SUM  country isFast rdCurrency
      WITH FRAME FRAME-A IN WINDOW TERMINAL-SIMULATION.
