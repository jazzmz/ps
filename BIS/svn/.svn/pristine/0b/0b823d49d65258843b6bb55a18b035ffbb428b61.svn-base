...
{for_form.i &withnew=yes &DataClass="f115-DataClass" }
   ...
   /* Перечень формул для отчета (формулы с '_p' - исключить) */
   IF INDEX(ENTRY(NUM-ENTRIES(formula.var-id,"_"),formula.var-id,"_"),"p") GT 0 THEN
      NEXT.
   ...
   DO:
      ...
      CASE mRaz:
         WHEN "r1" THEN
            ...
            .
         WHEN "r2" OR WHEN "r3" OR WHEN "s1" OR WHEN "s2" THEN
            ASSIGN
...
FOR EACH DataLine OF DataBlock NO-LOCK :
   ...
   FOR EACH ttFml WHERE NOT ttFml.itog :
      ...
      IF (ttFml.fml EQ TRIM(IF mDc EQ "rspr115" THEN DataLine.Sym3
                            ELSE IF mDc EQ "rn115" THEN REPLACE(DataLine.Sym1,"_p","")
                            ELSE DataLine.Sym1)   ) OR
         ...
         CASE mRaz:
            ...
            DO:
               ...
               IF vTmpInt  > 1 THEN
                  ...
                  .
               ELSE /* 1 кат.кач. */
                  IF ENTRY(NUM-ENTRIES(DataLine.Sym1,"_"),DataLine.Sym1,"_") EQ "p"  THEN /* Добавляем данные формул с "_p" */
                     ASSIGN
                        ttFml.Gr [15] = ttFml.Gr [15] + DataLine.Val [11]
                     .
            END.
