...
/** 
		...
		Производит экспорт данных в формате afx по кредитному договору физического лица
		(с валютной оговоркой).
		
		Бурягин Е.П., 12.10.2010, 09:45
...
FUNCTION exp_agree RETURNS CHAR (
			...
			INPUT arg5 AS CHAR, 
			INPUT arg51 AS CHAR, INPUT arg52 AS CHAR,
			INPUT arg53 AS CHAR, INPUT arg6 AS CHAR,
			...
			construction = construction + "summaeq=" + arg51 + CHR(13) + CHR(10).
			construction = construction + "eqrate=" + arg52 + CHR(13) + CHR(10).
			construction = construction + "eqcurr=" + arg53 + CHR(13) + CHR(10).
...
DEF VAR summaeq AS DECIMAL NO-UNDO.
DEF VAR eqrate AS DECIMAL NO-UNDO.
DEF VAR eqcurr AS CHAR NO-UNDO INIT "840".
...
: 
	...
	/** курс валюты на дату открытия договора */
	eqrate = 1.
	FIND LAST instr-rate WHERE
		instr-rate.instr-code = eqcurr AND
		instr-rate.since <= loan.open-date
		NO-LOCK NO-ERROR.
	IF AVAIL instr-rate THEN DO:
		eqrate = instr-rate.rate-instr.
	END.
	summaeq = ROUND(summa / 100 * eqrate, 2).	
