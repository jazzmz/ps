/** 
	pirloan018.p
	Формирование распоряжение по уменьшению лимита выдачи/задолженности.
	...
	:
		...
		IF TRUE THEN 
			DO:
				...
				amount = GetLoanLimit_ULL(loan.contract, loan.cont-code, end-date - 1, false)
									- GetLoanLimit_ULL(loan.contract, loan.cont-code, end-date, false).

				if loan.cont-code = "КЛ-01/10" and (end-date = 12/30/11 or end-date = 12/31/11) then amount = 19500000.
				IF amount > 0 THEN 
					DO:
						/** Выбираем информацию */
						...
						str[1] = "Уменьшить лимит " + (IF account BEGINS "91316" THEN "выдачи" ELSE "задолженности")
						         + ", учитываемый на внебалансовом счете №" + account + ", на сумму "
