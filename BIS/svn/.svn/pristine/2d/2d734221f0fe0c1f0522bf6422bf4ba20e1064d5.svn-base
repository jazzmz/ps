...
FUNCTION exp_client RETURNS CHAR (
			...
			construction = construction + "addressregistr=" + arg4 + CHR(13) + CHR(10).
			construction = construction + "acct=" + arg5 + CHR(13) + CHR(10).
			construction = construction + "docum=" + arg6 + CHR(13) + CHR(10).
			...
			IF AVAIL cust-corp THEN
				DO:
					PUT UNFORMATTED StrToWin_ULL(exp_client(
							...
							else  replace(cust-corp.name-short,chr(34),"``")
							), 
							GetXAttrValueEx("cust-corp",string(cust-corp.cust-id),"УчДокгр","") ,
							GetXAttrValueEx("cust-corp",string(cust-corp.cust-id),"ОГРН","") ,
							GetXAttrValueEx("cust-corp",string(cust-corp.cust-id),"RegPlace", "") ,
							account ,
							GetXattrValueEx("cust-corp",string(cust-corp.cust-id),"document-id","") + ": " + GetXattrValueEx("cust-corp",string(cust-corp.cust-id),"document","") + " Выдан: " + GetXattrValueEx("cust-corp",string(cust-corp.cust-id),"issue","") 
						)
					).
				END.
		END.
