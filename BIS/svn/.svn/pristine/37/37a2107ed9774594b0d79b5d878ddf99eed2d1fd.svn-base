...
/**
 * Распоряжение на учет суммы непогашенных процентов на счете 
 * просроченных процентов.
 * Бурягин Е.П., 27.07.2006 10:26
 *
...
def var ArrearsOfInterestSumma as decimal NO-UNDO.
...
def var PeriodBase as integer initial 365.
/** Процентная ставка */
...
def var ArrearsOfInterestAcct as char NO-UNDO.
...
FOR FIRST tmprecid NO-LOCK,
    ...
    :
		...
		{getdates.i}
		assign		beg_date = beg-date			end_date = end-date.

		/** Счета, участвующие в формировании распоряжения */
		LoanAcct = GetLoanAcct_ULL("Кредит",Loan,"Кредит",beg_date, ULLShowErrorMsg).
        
/* добавил Красков, тянулся не тот счет*/



          if loan.gr-riska <= 3 then ArrearsOfInterestAcct = GetLoanAcct_ULL("Кредит",Loan,"КредПр%",end_date, ULLShowErrorMsg). 
	  else
          ArrearsOfInterestAcct = GetLoanAcct_ULL("Кредит",Loan,"КредПр%В",end_date, ULLShowErrorMsg). 

/* конец вставки */

/*		ArrearsOfInterestAcct = GetLoanAcct_ULL("Кредит",Loan,"КредПр%,КредПр%В",end_date, ULLShowErrorMsg).*/
		 ...
		 */
					...
					ASSIGN
						...
						Summa = round(Balance * Rate / PeriodBase * Period, 2)
						TotalSumma = TotalSumma + Summa
						ArrearsOfInterestSumma = TotalSumma - PrePaySumma
						StrTable = StrTable + "    │" + STRING(ABS(Balance), ">>>,>>>,>>>,>>9.99")
							+ "│" + STRING(PeriodBegin,"99/99/9999") 
							+ "│" + STRING(PeriodEnd,"99/99/9999") 
							+ "│" + STRING(Period,">>>>>>>>")
					...
					StrTable = StrTable + "    └──────────────────┴──────────┴──────────┴────────┴────────┴──────────────────┘" + cr
					 	                  ...
					 	                  + "                                      ИТОГО (проценты к оплате):" + STRING(ArrearsOfInterestSumma,">>>,>>>,>>>,>>9.99") + cr.
		
		/** Формирование и печать распоряжения */
		
		/* Сумма прописью */
		Run x-amtstr.p(ArrearsOfInterestSumma, loan.currency, true, true, output SummaStr[1], output SummaStr[2]).
  	SummaStr[1] = SummaStr[1] + ' ' + SummaStr[2].
		Substr(SummaStr[1],1,1) = Caps(Substr(SummaStr[1],1,1)).
 
		/** Текс распоряжения */
		MainText[1] = "Прошу учесть на счете №" + ArrearsOfInterestAcct + " сумму непогашенных в срок "
		            + "процентов за кредит за период с " + STRING(beg_date, "99/99/9999") + " по " 
		            + STRING(end_date, "99/99/9999") + " вкл. в размере " 
		            + STRING(ArrearsOfInterestSumma,">>>,>>>,>>>,>>9.99") + "(" + SummaStr[1] + ") по кредитному договору №"
		            + cDocID
		            + ', заключенному между ' + cBankName + ' и ' + ENTRY(2, LoanInfo) + ".".
		{wordwrap.i &s=MainText &l=74 &n=10}
		
		{setdest.i}
		
		put unformatted SPACE(60) "В Департамент 3" SKIP
								SPACE(60) cBankName SKIP(3)
