...
def var payment_text as char no-undo. /** our & your payment поля. разные в зависимости от валюты */
...
refer = date_sd + "/" + substring(loan.doc-num,6,5).
...
/** Buryagin added the next code at 05.12.2007 18:44:54 */
payment_text = "".
if loan.currency = "" then
	do:
		payment_text = "OUR PAYMENT:" + chr(10) +
        "         ACCORDING YOUR INSRUCTIONS" + chr(10) + chr(10) +
        "YOUR PAYMENT:" + chr(10) + 
        "         /30101810500000000491" + chr(10) +
        "               WITH PROMINVESTRASCHET, MOSCOW" + chr(10) +
        "               BIC " + string(FGetSetting("БанкМФО",?,""), "x(9)") + ", INN " + 
                            string(FGetSetting("ИНН",?,""), "x(10)") + "," + chr(10) +
        "               ACC " + string(acc_1, "x(20)") + "," + chr(10).
        IF loan.contract = "Кредит" THEN 
        	payment_text = payment_text + 
        		"               INT ACC" + string((if perehod eq yes then int_3 + " RUB " + summa_proc_nach else int_2), "x(40)") + chr(10) + 
        		"                  " + string((if perehod eq yes then "ACC " + int_2 + " RUB " + summa_proc_os else ""), "x(40)") + chr(10). 
	end.
else
	do:
		
		payment_text = "OUT PAYMENT:" + chr(10).
		
		/** Найдем исходящий платеж для текущей сделки */
		find first out-payment where out-payment.contract = loan.contract
		                         and out-payment.cont-code = loan.cont-code
		                         and out-payment.class-code = "mml-pr-obl"
		                         and out-payment.end-date = loan.open-date 
		                         no-lock no-error.
	  /** Найдем теперь связанного субъекта для пратежа */
	  find first cust-role where cust-role.file-name = "term-obl"
	                         and cust-role.surrogate = out-payment.contract + "," + out-payment.cont-code + "," + string(out-payment.idnt) + "," + string(out-payment.end-date) + "," + string(out-payment.nn)
	                         and cust-role.class-code = "Acct-With-Inst"
	                         and cust-role.open-date le out-payment.end-date
	                         and (cust-role.close-date ge out-payment.end-date
	                              or
	  ...
	  if avail cust-role then 
	  	do:
			 payment_text = payment_text + "                              " + cust-role.cust-name + " ACC " + cust-role.corr-acct + " " + cust-role.cust-code + chr(10) + chr(10).
	  	end.
	  else
	  	do:
			 payment_text = payment_text + "          <not available info! нужен участник платежа с ролью Acct-With-Inst>" +  chr(10) + chr(10).
	  	end.

		payment_text = payment_text + 
			"YOUR PAYMENT:" + chr(10) +
			"                              PROMINVESTRASCHET, MOSCOW, SWIFT BPIRRUMM" + chr(10) +
			"                              ACC 0103971412 with VTB Bank (Deuchland) AG," + chr(10) +
			"                              Frankfurt am Main, Germany, SWIFT OWHBDEFF, with" + chr(10) +
			"                              CITIUS33XXX" + chr(10).
...
{setdest.i}
    put unformatted
        ...
        "WE CONFIRM " + (IF loan.contract = "Кредит" THEN "LENDING" ELSE "BORROWING") + " FUNDS IN DEPOSIT WITH YOU" skip
        ...
        "TRADE DATE:"    string(loan-cond.since,"99.99.9999") 
        								 format "x(10)"  at 19 skip
        "VALUE DATE:"    string(loan-cond.since,"99.99.9999") 
        							   format "x(10)"  at 19 skip
