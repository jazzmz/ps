/*
   ...
   Parameters:
     ...
     Modified: 05/07/2011 Zorin
     /*Выгрузка проводок без платежей сам-себе */
*/
{globals.i}
{tmprecid.def}
{pir_exf_exl.i}
...
FOR EACH tmprecid
 ...
 find first acct where acct.acct = acct-db no-lock no-error .  
 if avail (acct) then do:
    iDbCust = acct.cust-id . 
    cDbCat = acct.cust-cat . 
  end .

 find first acct where acct.acct = acct-cr no-lock no-error .  
 if avail (acct) then do:
    iCrCust = acct.cust-id . 
    cCrCat = acct.cust-cat .  
 end.
 
 IF (iDbCust EQ iCrCust) AND (cDbCat EQ cCrCat) then next .
 IF (cDbCat = "В") then next .
