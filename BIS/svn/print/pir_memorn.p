...
/*****************************************
 *				         *
 * Женя, накрутил этот отчет             *
 * не слабо. Его или переделывать        *
 * или ковырять дальше.                  *
 * Пока попытаюсь поковырять, так        *
 * как не охота убивать кучу времени.    *
 *****************************************
 *                                       *
 * Создаю временную таблицу в эту        *
 * таблицу буду размещать маркеры.       *
 * Фактически буду фиксировать намерения *
 * выгрузки в электронный архив.         *
 * Затем по факту буду заполнять ДР.     * 
 *                                       *
 *                                       *
 *				         *
 *****************************************
...
FOR EACH tmprecid NO-LOCK,
   ...
   FOR EACH op-entry WHERE op-entry.op = op.op NO-LOCK,
      FIRST doctypes WHERE CAN-DO(doctypes.doc-type, op.doc-type)
         ...
         NO-LOCK:
        /*приложение к м/о #1372 */
        if drpril = "______" then
	     	drpril = GetXAttrValueEx("op-entry",STRING(op-entry.op) + "," + STRING(op-entry.op-entry),"Приложение","______").
     	/*конец #1372 */
...
*/
	...
	/*********************************
         *
         * Здесь производим пометку
         * о выгрузке документа в электронный
         * архив. Таких места два. Это первое.
         *
         **********************************
         *
	 * Автор         : Маслов Д. А.
...
&ENDIF
	...
	/*** Конец #790 ***/
        END.
         ...
         /****************************
          * Если проводка по депо, то заменяем на
          * количество.
          ****************************/
...
*/
	...
	/*********************************
         ...
         * Здесь производим пометку
         * о выгрузке документа в электронный
         * архив. Таких места два. Это второе.
         *
         **********************************
         *
	 * Автор         : Маслов Д. А.
...
&ENDIF               
	/*** Конец #790 ***/
      ...
      /*************************************
       * Для депо это неверно, так        *
       * как там вместо суммы выступает   *
       * количество.                      *
       ************************************/
...
/*приложение к м/о #1372 */
		...
		msgPril = TRUE.
		MESSAGE "Печатать в приложении " drpril "?"
...
/***********************************************
 * Производим удаление дублирующих
 * записей из таблиц.
 * **********************************************
 *
 * Автор: Маслов Д. А.
 * Дата создания: 16:37 28.07.2011
 * Заявка: #736
 *
 ***********************************************/
...
CREATE rep2. k = k + 1. ASSIGN rep2.id = k rep2.docid=doc.id.
ASSIGN rep2.str2 = "#tab" + "МЕМОРИАЛЬНЫЙ " + (IF "{&MEM_TYPE}" <> "" THEN "{&MEM_TYPE} " ELSE "") + "ОРДЕР    N" + doc.doc-num + FILL(" ", 20 - LENGTH(IF "{&MEM_TYPE}" <> "" THEN "{&MEM_TYPE} " ELSE "")) + STRING(doc.doc-date, "99.99.9999").
CREATE rep2. k = k + 1. ASSIGN rep2.id = k rep2.docid=doc.id.
...
/* По #3767 10.09.2013 Гончаров А.Е.*/ 
	if can-do (FGetSetting("ОперДень","Befcl",""),op.op-kind) then theUser = "АВТ".
/* По #3767 */ 
   ...
   IF rep2.id = row-count THEN DO:
      ...
      rep2.str2 = rep2.str2 +
         CHR(10) + CHR(10) + "#tab─────────────────────────────────────────────────────────────────────────────" 
         + CHR(10) + "#tab" + STRING("Приложение: ______" + drpril + "_" + ndoc + "___________ документов на ________" + nlist + "_______ листах.").
   END.
...
DELETE OBJECT oSysClass1.
  ...
  END.
  ELSE DO:
   		                 MESSAGE COLOR WHITE/RED "ОШИБКА ВЫГРУЗКИ ДОКУМЕНТА " SKIP
			         ERROR-STATUS:GET-MESSAGE(1)
				 VIEW-AS ALERT-BOX TITLE "[ОШИБКА #790]".
...
&ELSE
MESSAGE "Выгрузка в архив завершена!" VIEW-AS ALERT-BOX.
&ENDIF
