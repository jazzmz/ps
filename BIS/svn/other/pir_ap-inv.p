/*
    ...
    Copyright: (C) 1992-2007 ЗАО "Банковские информационные системы"
     Filename: pir_ap-inv.p
      Comment: Инвентаризационная опись по страницам переделана из ap-inv.p
		используемый шаблон передается параметром
   Parameters:
         Uses:
      Used by:
...
/*----------------------------------------------------------------------------*/
   ...
   DEF VAR v_user-proc_procedure AS CHAR NO-UNDO.
   IF i_user-proc_procedure = "" OR i_user-proc_procedure = ? THEN DO:
     IF   work-module EQ "ОС"
	THEN v_user-proc_procedure = "osinv".
     ELSE IF work-module EQ "НМА"
	THEN v_user-proc_procedure = "nmainv".
     ELSE IF CAN-DO("МБП,СКЛАД", work-module)
	THEN v_user-proc_procedure = "mbpinv".
   END.
   ELSE v_user-proc_procedure = i_user-proc_procedure.
     
   RUN norm-rpt.p ("" , v_user-proc_procedure, in-branch-id, in-beg-date, in-end-date).
END.
/*----------------------------------------------------------------------------*/
