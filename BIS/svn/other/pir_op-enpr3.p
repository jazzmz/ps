...
def var valcr as char format 'x(3)' no-undo.
def var valdb as char format 'x(3)' no-undo.
def var kod as char format 'x(3)' no-undo.
...
for each tmprecid no-lock,
   ...
   form header
              ...
              format "x(45)"
              page-number form "Листzz9" to 126 skip(2).
   ...
   if substr(adb,7,3) = "810" then kod = GetXattrValueEx("op",
                                               string(op-entry.op),
					       "country-rec",
					       "").
    
   if substr(acr,7,3) = "810" then kod = GetXattrValueEx("op",
                                               string(op.op),
					       "country-send",
					       "").
...
/*    op-entry.user-id */
  ...
  accum
    op-entry.amt-cur (sub-total by op-entry .op-date by op-entry.currency)
    op-entry.amt-cur (count by op-entry.op-date by op-entry.currency)
  ...
  if lastofuserid then do:
     ...
     underline op.doc-num op-entry.amt-cur op-entry.amt-rub.
     display
        (accum count by op-entry.op-date op-entry.amt-rub) format ">>>>9" 
	   @ op.doc-num
	(accum total by op-entry.currency op-entry.amt-cur) @ op-entry.amt-cur
	(accum sub-total by op-entry.currency op-entry.amt-cur) @ op-entry.amt-cur
        (accum total by op-entry.op-date op-entry.amt-rub) @ op-entry.amt-rub
	(accum sub-total by op-entry.op-date op-entry.amt-rub) @ op-entry.amt-rub.
     down 2.
  end.
  ...
  if lastuserid and j > 1 then do:
	...
	(accum sub-count op-entry.amt-rub) format ">>>>9" @ op.doc-num
        (accum sub-total op-entry.amt-cur) @ op-entry.amt-cur 
        (accum sub-total op-entry.amt-rub) @ op-entry.amt-rub
     .
  end.
