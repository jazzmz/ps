...
&SCOP Qu2 OPEN QUERY q PRESELECT                                               ~
            ...
            FIRST op-entry OF op                                                  ~
                    WHERE (op-entry.amt-rub EQ vamt-rub OR NOT enter-summa)       ~
                      ...
                      AND (op-entry.symbol eq vsymbol OR NOT enter-symbol)        ~
...
DEF VAR vsymbol        LIKE op-entry.symbol   NO-UNDO.
...
DEF VAR enter-symbol    AS LOG INIT YES        NO-UNDO.
...
DEFINE BROWSE b QUERY tq
  DISPLAY
    ...
    tdoc.acct-db  FORMAT "x(20)"             COLUMN-LABEL 'СЧЕТ КЛИЕНТА'
    tdoc.symbol   FORMAT "x(2)"              COLUMN-LABEL 'КАССОВЫЙ СИМВОЛ'
    /* tdoc.acct-cr  FORMAT "x(20)"             COLUMN-LABEL 'СЧЕТ ПОЛУЧАТЕЛЯ'  */
...
DEF FRAME f
 ...
 "┌────────────────────────────[ ПОЛЯ ДЛЯ ВВОДА ]──────────────────────────────┐" SKIP
 "│ NN ДОК    СУММА                 СЧЕТ КЛИЕНТА            КАССОВЫЙ СИМВОЛ    │" SKIP
 "│ ------    ---------------      --------------------     ---------------    │" SKIP
 "│"  vdoc-num
      vamt-rub AT COL 13 ROW 6 FORMAT ">>>>>>>>>>>>.99"
      vacct-db AT COL 34 ROW 6 FORMAT "x(20)"     
      vsymbol  AT COL 64 ROW 6 FORMAT "x(2)"                                  "│" TO 78 SKIP
 "└────────────────────────────────────────────────────────────────────────────┘" SKIP
 b
...
DO TRANSACTION ON ERROR  UNDO CHECK-DOCS, LEAVE CHECK-DOCS
    ...
    IF NOT enter-symbol    THEN DISABLE vsymbol   WITH FRAME f.
...
PROCEDURE get-set:
   {get_set2.i "ПовторныйВводЧ" "НачСтатус"} xop-status     = setting.val.
   {get_set2.i "ПовторныйВводЧ" "КонСтатус"} xop-status2    = setting.val.
   {get_set2.i "ПовторныйВводЧ" "InputProc"} input-proc     = setting.val.
   {get_set2.i "ПовторныйВводЧ" "DocNum"}    enter-docnum   = lookup(setting.val,'yes,да,true') <> 0.
   {get_set2.i "ПовторныйВводЧ" "PlatAcct"}  enter-platacct = lookup(setting.val,'yes,да,true') <> 0.
   {get_set2.i "ПовторныйВводЧ" "Summa"}     enter-summa    = lookup(setting.val,'yes,да,true') <> 0.
   {get_set2.i "ПовторныйВводЧ" "КасСим"}    enter-symbol = lookup(setting.val,'yes,да,true') <> 0.
END.
