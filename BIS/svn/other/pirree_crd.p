...
DEFINE OUTPUT PARAMETER oKolCrd    AS   INTEGER           NO-UNDO.

DEF VAR vFile-bnk AS CHAR INIT "crd.txt" NO-UNDO.
...
FOR EACH ttRee ,
    EACH ttReeLoan WHERE ttReeLoan.SysNum = ttRee.SysNum
                   AND   ttReeLoan.Symbol = "к" BY id:
    oKolCrd =  oKolCrd + 1.
    PUT STREAM bnk UNFORMATTED
        /* string(ttRee.id) */ string(INT(ttRee.SysNum)) + "^" +         /*Номер ПП*/
