...
do:
  ...
  DO:
     for each cust-corp no-lock:
        ...
        assign
           ...
           */
           clis.cust-stat = ""
           clis.name-corp = cust-corp.name-short
        .
        release clis.
     end.
  END.
  ...
  cBankName = getBranchXAttrValue(dept.branch,"Банк").
	...
	FOR EACH acct WHERE acct.acct BEGINS "4" BY (STRING(acct.bal-acct) + SUBSTRING(acct.acct,17,4))   : 
	FOR	FIRST clis 
	WHERE acct.cust-id = clis.cust-id AND CAN-DO(cAcctMask, acct.acct) AND
      ...
      IF LastCustID NE clis.cust-id THEN DO:
         ...
         IF Line-Count <> 1 AND lIsNewPageClient /* каждого клиента на новой странице */
            THEN PAGE.
           /* печатаем заголовок */
        cClientName[1] = TRIM(clis.cust-stat + ' ' + clis.name-corp).
        {wordwrap.def}
