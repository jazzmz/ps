...
message "®¤¢®¤¨âì ¨â®£¨ ¯® ®â¢¥âáâ¢¥­­®¬ã ¨á¯®«­¨â¥«î ?" VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO update suppress.

{get-user.i}

IF NOT list-id > '' THEN DO:
   MESSAGE "¥ ¢ë¡à ­ë á®âàã¤­¨ª¨!" 
   VIEW-AS ALERT-BOX.
   UNDO, RETRY.
END.
...
FOR EACH tmprecid, FIRST acct WHERE recid(acct) = tmprecid.id NO-LOCK,
    ...
    form
        ...
        header caps(br-name) format "x(55)" SKIP(2)
               ...
               "‡€"  date-rep "„-’ " long-acct SKIP
	       "ŠŽ’Ž‹… " _user._user-name SKIP(2)
    ...
    if not can-do(spbal,string(xacct.bal-acct,"99999")) then do:
       accumulate op-entry.amt-rub (total by acct.acct)
		  ...
		  op-entry.amt-rub (count by op.user-inspector)
                  op-entry.amt-rub (total by op.user-id)
                  op-entry.amt-rub (count by op.user-id)
    ...
    if last-of(op.user-id) and suppress and
       (accum total by op.user-id op-entry.amt-rub) ne 0 then do:
       ...
       display "ˆ’ŽƒŽ" @ op.doc-num
               ...
               accum total by op.user-id op-entry.amt-rub @ op-entry.amt-rub.
    ...
    if last-of(op.user-inspector) and
