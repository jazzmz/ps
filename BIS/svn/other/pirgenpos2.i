/*
   ...
   Parameters: см. ниже
     ...
     Modified: 15/08/2007 Kuntash Изменил в acct-pos gop-status сначала на В потом на крыж, чтобы считался планируемый остаток                         
...
DEFINE VARIABLE mAllContract   AS LOGICAL         NO-UNDO. /*чтобы в цикле вычислений не делать*/
DEFINE VARIABLE mAllwaysHeader AS LOGICAL INIT NO NO-UNDO.
...
DEFINE VARIABLE mVedBalSch     AS CHARACTER       NO-UNDO.
DEFINE VARIABLE mRights-pos    AS LOGICAL         NO-UNDO.
DEFINE VARIABLE mAcct-look     AS CHARACTER       NO-UNDO.
DEFINE VARIABLE mAcctYes       AS CHARACTER       NO-UNDO.
DEFINE VARIABLE mAcctNo        AS CHARACTER       NO-UNDO.
DEFINE VARIABLE ii             AS INTEGER         NO-UNDO.
DEFINE VARIABLE mDtNr          AS DATE            NO-UNDO. /* параметр Дата_НР */
DEFINE VARIABLE mDtDPD         AS DATE            NO-UNDO. /* реквизит ДПД-НР на счете */

ASSIGN
   mRights-pos = getThisUserXAttrValue("ПросмотрОст")  EQ "Да"
   mAcct-look  = getThisUserXAttrValue("Л/С")
   mVedBalSch  = FGetSetting ("Отчеты","ВедБалСчета","!*")
   mDtNr       = DATE(FGetSetting ("Дата_НР", ?, ""))
.

/* разбор масок счетов для скрытия */
IF mRights-pos THEN DO:
   DO ii = 1 TO NUM-ENTRIES(mAcct-look):
      IF ENTRY(ii,mAcct-look) BEGINS "!" THEN
         {additem.i mAcctNo SUBSTRING(ENTRY(ii,mAcct-look),2)}
      ELSE
         {additem.i mAcctYes ENTRY(ii,mAcct-look)}
   END.
END.

IF mAcctYes EQ "" THEN mAcctYes = "*".

&IF (DEFINED (nobalacctmask) EQ 0 AND DEFINED(obsacred) EQ 0) &THEN
   ...
   IF NUM-ENTRIES(iBalAcctMask) EQ 2 THEN
      ...
      .
   IF mVedBalSch NE "!*" THEN DO:
      {additem.i iBalAcctMask mVedBalSch}
   END.
&ENDIF

&IF DEFINED (obsacred) NE 0 &THEN
   DEFINE INPUT  PARAMETER iBalAcctMask AS CHARACTER  NO-UNDO.
   mNumEntries = NUM-ENTRIES(iBalAcctMask,";").
   ASSIGN 
      mContract    = IF mNumEntries GE 2 THEN ENTRY(2,iBalAcctMask,";") ELSE ""
      mAcctCat     = IF mNumEntries EQ 3 THEN ENTRY(3,iBalAcctMask,";") ELSE ""
      iBalAcctMask = ENTRY(1,iBalAcctMask,";")
   .

   IF mContract    EQ "" THEN mContract    = "Кредит".
   IF mAcctCat     EQ "" THEN mAcctCat     = "b".
   ...
   mAllContract = IF LENGTH(mAcctCat) EQ 1 THEN NO ELSE YES.

   &GLOBAL addwhere AND CAN-DO(mContract,acct.contract) AND CAN-DO(mAcctCat,acct.acct-cat)
&ENDIF
...
&IF DEFINED(obsacred) EQ 0 &THEN
   mAllContract = NO.
&IF DEFINED(nobalcur) EQ 0 &THEN
   IF in-acct-cat         NE ? OR
      STRING(in-acct-cat) NE "" THEN
      mAllContract = YES.
&ENDIF
&ENDIF
...
&ELSE
                 "ВЕДОМОСТЬ ПЛАНИРУЕМЫХ ОСТАТКОВ ЗА "
&ENDIF
...
mDtDPD = DATE(GetXAttrValueEx("acct",acct.acct + "," + acct.currency,"ДПД-НР","")).

/*Добавил Кунташев понижаю gop-status до В */
RUN acct-pos IN h_base (acct.acct, acct.currency, beg-date, end-date,"В").
/*
{ifdef {&turnover} {&curturn}}
    RUN acct-pos IN h_base (acct.acct, acct.currency, beg-date, end-date, ?).
{else} /**/
    RUN acct-pos IN h_base (acct.acct, acct.currency, end-date, end-date,"В").
{endif} */
...
&IF DEFINED (NoBalAcctMask) EQ 0 &THEN
    IF     NOT CAN-DO (iBalAcctMask, STRING (acct.bal-acct,"99999")) 
       AND NOT CAN-DO (mAcctNo,STRING(acct.acct))
       AND     CAN-DO (mAcctYes,STRING(acct.acct))
    THEN DO:
&ENDIF
...
&ENDIF
      ...
      IF acct.currency NE ""  AND
         ...
         lastmove = lastcurr.
      /* корректируем дату последнего движения */
      IF lastmove EQ mDtNr THEN
         IF mDtDPD NE ? AND mDtDPD LT mdtBegDate THEN
            lastmove = mDtDPD.
