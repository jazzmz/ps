...
kurs = 1.
for each tmprecid, 
	first bbanks where RECID(bBanks) EQ tmprecid.id.
	/*display bbanks.bank-id.*/
	for each bAcct where bBanks.bank-id=bAcct.cust-id and (bAcct.acct begins "30110" or bAcct.acct begins "30114" or bAcct.acct begins "30402") and bAcct.cust-cat eq 'Б' and bAcct.close-date = ? NO-LOCK.
	        ...
	        IF acct_cur="840" OR acct_cur="" THEN 
	           do.
	                acct_cur = "USD".
	                kurs = kursusd.
	           End.
	        IF acct_cur="978" OR acct_cur="" THEN 
	           do.
	                acct_cur = "EUR".
	                kurs = kurseur.
	           end.
		posRur = posVal * kurs.
	        ...
	        oTable1:addCell(posRur).
	        oTable1:addCell(grrisk).
	        oTable1:addCell(prrisk).
	        oTable1:addCell(posRur * prrisk). 
