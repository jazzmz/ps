...
if (LastMonDate(currDate) <> PrevWorkDay(LastMonDate(currDate) + 1)) 
    and (currDate =  PrevWorkDay(LastMonDate(currDate) + 1)) then lastworkdate = yes. else lastworkdate = no.
...
if not lastworkdate then 
	baseCalc = oSysClass:convert2rur(INT(loan.currency),getRsrvBase(loan.cont-code,currDate,"%"),currDate).
else
	baseCalc = oSysClass:convert2rur(INT(loan.currency),getRsrvBase(loan.cont-code,LastMonDate(currDate),"%"),currDate).
...
oClient = new TClient(loan.cust-cat,loan.cust-id).
        baseCalc = oSysClass:convert2rur(INT(loan.currency),getRsrvBase(loan.cont-code,currDate,"bad%"),currDate).
	IF baseCalc <> 0 OR showZero THEN DO:
	    i = i + 1.
	oTable2:addRow().
	 oTable2:addCell(i).
	 oTable2:addCell(loan.cont-code + (IF loan.currency <> oSysClass:getSetting("КодНацВал") AND loan.currency <> "" THEN "*" ELSE "")).
	 oTable2:addCell(oClient:name-short).
...
oClient = new TClient(loan.cust-cat,loan.cust-id).
	...
	oTable3:addRow().
	 oTable3:addCell(i).
	 oTable3:addCell(loan.cont-code + (IF loan.currency <> oSysClass:getSetting("КодНацВал") AND loan.currency <> "" THEN "*" ELSE "")).
	 oTable3:addCell(oClient:name-short).
...
oClient = new TClient(loan.cust-cat,loan.cust-id).
	...
	IF baseCalc <> 0 OR showZero THEN DO:
	    ...
	    oTable4:addRow().
	      oTable4:addCell(i).
	      oTable4:addCell(loan.cont-code  + (IF loan.currency <> oSysClass:getSetting("КодНацВал") AND loan.currency <> "" THEN "*" ELSE "")).
	      oTable4:addCell(oClient:name-short).
...
oClient = new TClient(loan.cust-cat,loan.cust-id).
	 ...
	 oTable5:addRow().
	   oTable5:addCell(i).
	   oTable5:addCell(loan.cont-code + (IF loan.currency <> oSysClass:getSetting("КодНацВал") AND loan.currency <> "" THEN "*" ELSE "")).
	   oTable5:addCell(oClient:name-short).
...
oClient = new TClient(loan.cust-cat,loan.cust-id).
	...
	IF baseCalc <> 0 OR showZero THEN DO:
	    ...
	    oTable6:addRow().
	      oTable6:addCell(i).
	      oTable6:addCell(loan.cont-code  + (IF loan.currency <> oSysClass:getSetting("КодНацВал") AND loan.currency <> "" THEN "*" ELSE "")).
	      oTable6:addCell(oClient:name-short).
