/*
#4156. Погашение ОД по срокам погашения для ф.125. ПК
Никитина Ю.А. 03.12.2013
*/
      ...
      if loan.close-date eq ? then do:
         ...
         if loan.end-date GE end-date then do:
            ...
            for each term-obl where term-obl.cont-code eq Loan.cont-code and term-obl.contract eq Loan.contract
               ...
               no-lock:
               find first mTermObl where mtermobl.contcode eq term-obl.cont-code and mtermobl.contract eq term-obl.contract 
                  and mtermobl.enddate eq term-obl.end-date
                  no-lock no-error.
               if not avail mTermObl then do:
                  create mTermObl.
                  ASSIGN
                     mTermObl.ContCode = term-obl.cont-code
            ...
            if not flag then do:
               find first mTermObl where mtermobl.contcode eq loan.cont-code and mtermobl.contract eq loan.contract 
                  and mtermobl.enddate eq lend-date
                  no-lock no-error.
               if not avail mTermObl then do:
                  create mTermObl.
                  ASSIGN
                     mTermObl.ContCode = loan.cont-code
...
for each mTermObl no-lock by mTermObl.contcode:
   ...
   if not avail mTermObl2 then do:
      ...
      ASSIGN
         ...
         mTermObl2.closedate = mTermObl.enddate
      .
      ...
      mTermObl2.kolday = loan.end-date - end-date + 1.
      ...
      for each bloan where bloan.cont-code eq mTermObl.ContCode and bloan.contract eq mTermObl.contract 
         ...
         if GrRisk = 1 or GrRisk = 2 then do:
            ...
            if bloan.end-date GE end-date then do:
               if bloan.open-date eq (end-date - 1) then do:
                  RUN SetSysConf IN h_base ("NoProtocol","YES").
                  RUN SetSysConf IN h_base ("PUT-TO-TMESS","").
                  {setdest.i &stream="stream err" &filename='_spool_recalc.tmp' &append = "APPEND"}
                  {get_meth.i  'Calc' 'loanclc'}
...
/*   end.*/
   ...
   cXL[j] = cXL[j] + XLRowInFormat(45,"s80") + XLCell(string(istr)) + XLCell(mTermObl2.contcode) 
                   ...
                   + XLCell(string(mTermObl2.closedate,"99.99.9999"))
                   + XLNumCell(mTermObl2.kolday)
...
outfile = ("/home2/bis/quit41d/imp-exp/users/" + LC(UserID("bisquit")) + "/125_credit_od_pk.xls").
/*outfile = "./125_credit_od_pk.xls".*/
