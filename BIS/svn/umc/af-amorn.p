...
FUNCTION Can-Amort RETURNS LOGICAL :
   ...
   FOR
      EACH  op-kind WHERE
        ...
        AND op-kind.proc   MATCHES "*amorn*"
         NO-LOCK,
      ...
      FIRST op WHERE
       ...
       MESSAGE
         "Найден документ по начислению амортизации"      SKIP
         "от" op.op-date "№" op.doc-num                   SKIP(1)
         "Возможно, начисление амортизации в этом месяце" SKIP
...
/* Общие проверки на проведение списания 10%. */
FUNCTION Can-Spis RETURNS LOGICAL :
   ...
   END.
	...
	MESSAGE
       "Не найден документ по единовременному списанию"      SKIP
         "Возможно, единовременное списание в этом месяце" SKIP
         "не проведено! Продолжать?"
      VIEW-AS ALERT-BOX QUESTION BUTTONS OK-CANCEL UPDATE vOK.
      
      RETURN vOK.

END FUNCTION. /* Can-Spis */
...
RUN browseld.p (work-module, "", "", "", 4).
/* RUN loumcflt.p (in-op-date).*/
...
DO TRANSACTION WITH FRAME opreq
   ...
   DO ON ERROR  UNDO gen, LEAVE gen
      ...
      FOR
         ...
         {debuger.i '"af-amorn.p:начислено"' "inter"}
