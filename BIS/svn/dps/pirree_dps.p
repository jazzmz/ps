...
/* VNE 01/06/2009
   ...
   регистрации в качестве адреса для почтовых уведомлений */
	...
	RUN GetAddress(INPUT "АдрУвед", person.person-id,OUTPUT adr_uved).
	RUN GetAddress(INPUT "АдрФакт", person.person-id,OUTPUT adr_fakt).
...
PROCEDURE Get_PersonInfo.
   ...
   */
   ...
   RUN GetAddress(INPUT "АдрУвед", iPersId, OUTPUT outadr_uved).
   RUN GetAddress(INPUT "АдрФакт", iPersId, OUTPUT outadr_fakt).
