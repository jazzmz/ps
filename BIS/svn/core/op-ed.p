...
PROCEDURE CheckBenAcct :
   ...
   DO WITH FRAME edit:
      ...
      IF oOk AND vBenLen LE 20 AND mBankCodeType EQ 'МФО-9' THEN 
      DO:
   ...
   DO:
      CASE vResult:
         ...
         WHEN {&EGMissingCorrAcct} OR WHEN {&EGBadCorrAcct} THEN 
            IF mBankCodeType EQ "МФО-9" OR mBankCodeType EQ "" THEN
            DO:
               MESSAGE SKIP vMsg SKIP(1) VIEW-AS ALERT-BOX ERROR BUTTONS OK.
...
END.
/*****************
 * Запреты на исправление документов
 * PIR
 *****************/

 /*** Линкованные документы ***/
{pir-op-ed-1.i}
