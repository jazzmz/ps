...
DEFINE VARIABLE DopSogl     AS DATE  NO-UNDO. /* Дата окончания доп.соглашения */
...
DEFINE TEMP-TABLE tt-comm-rate NO-UNDO
   ...
   FIELD DopSogl     AS DATE FORMAT "99/99/9999"
...
DEFINE TEMP-TABLE tt-name NO-UNDO
   ...
   FIELD idx1       AS INT64 FORMAT ">>9"
   FIELD idx2       AS INT64 FORMAT ">>9"
...
FOR EACH commission 
   ...
   FOR LAST xcomm-rate
      WHERE xcomm-rate.commission EQ commission.commission
        AND xcomm-rate.filial-id = shfilial
        AND xcomm-rate.branch-id = ""
   ...
   IF vLog THEN
      DO: 
	     ...
	     **********************************/
            ...
            ASSIGN 
               DopSogl = DATE(GetXattrValueEx("acct", acct.acct + "," + acct.currency,
                                       "DopSoglSrok",
                                       "")).

      	 IF DopSogl <> ? 
      	 AND DopSogl GE beg-date
         AND DopSogl LE end-date THEN 
 ...
 END.   /* IF vLog THEN     */                                                      
    ...
    IF vLog THEN          /* Если счет найден, начинаем сбор информации по нему. */
      DO:
         ...
         ASSIGN 
            npp = npp + 1. /* П/П - номер первого счета. Равен единице.*/

          CREATE tt-comm-rate. /* Создаем запись для отчета. */
          ASSIGN
            tt-comm-rate.DopSogl     = DATE(GetXattrValueEx("acct", acct.acct + "," + acct.currency,
                                       "DopSoglSrok",
                                       ""))
...
PUT UNFORMATTED "┌───────┬────────────────────┬─────────────────────────────┬──────────────────────────────────────────────────────────────────┬────────────┬─────────────┐" skip.
PUT UNFORMATTED "│   N   │        Номер       │                             │                        Базовый тариф                             │Индивидуаль │     Срок    │" skip.
PUT UNFORMATTED "│  п.п. │   лицевого счета   │    Наименование счета       ├──────────┬────────────────────────────┬───────────────┬──────────┤   ная      │   окончания │" skip.
PUT UNFORMATTED "│  (по  │                    │                             │   Код    │         Наименование       │   Минимум     │  Ставка  │  ставка    │действия доп.│" skip.
PUT UNFORMATTED "│счетам)│                    │                             │          │                            │               │          │            │  соглашения │" skip.
PUT UNFORMATTED "├───────┼────────────────────┼─────────────────────────────┼──────────┼────────────────────────────┼───────────────┼──────────┼────────────┼─────────────┤" skip.
/*MESSAGE beg-date VIEW-AS ALERT-BOX BUTTONS OK.
MESSAGE DopSogl VIEW-AS ALERT-BOX BUTTONS OK. 
MESSAGE end-date VIEW-AS ALERT-BOX BUTTONS OK.
IF DopSogl GE beg-date 
  AND DopSogl LE end-date THEN*/
...
PUT UNFORMATTED "│"IF tt-comm-rate.acct NE mAcct THEN string(npp,">>>>>>9") ELSE '       ' "│" 
                ...
                "│   " tt-comm-rate.DopSogl FORMAT "99/99/9999"
                "│" skip.
...
PROCEDURE vcan-do.
   ...
   DEFINE VARIABLE i AS INT64  INIT 1  NO-UNDO.
   vLog = NO.
