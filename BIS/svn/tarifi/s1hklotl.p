...
DEFINE VARIABLE npp         AS INT64 INIT 0 NO-UNDO. /* Номер по порядку */
           ...
           IF AVAILABLE acct THEN
            ...
            ASSIGN 
               DopSogl = DATE(GetXattrValueEx("acct", acct.acct + "," + acct.currency,
                                       "DopSoglSrok",
                                       "")).
/*              message dopsogl VIEW-AS ALERT-BOX.*/
/*      	 IF DopSogl <> ? 
     ...
     vLog = NO.
      ...
      DO:
         ...
         ASSIGN
			...
			tt-comm-rate.date        = comm-rate.since
         .
...
PUT UNFORMATTED "│  п.п. │   лицевого счета   │    Наименование счета       ├──────────┬────────────────────────────┬───────────────┬──────────┤   ная      │   Дата     │   окончания │" skip.
PUT UNFORMATTED "│  (по  │                    │                             │   Код    │         Наименование       │   Минимум     │  Ставка  │  ставка    │ заведения  │действия доп.│" skip.
...
PUT UNFORMATTED "│"IF tt-comm-rate.acct NE mAcct THEN string(npp,">>>>>>9") ELSE '       ' "│" 
				...
				"│ " IF tt-comm-rate.ind-comm LT 0 THEN "          " ELSE string(tt-comm-rate.date,"99.99.9999")
