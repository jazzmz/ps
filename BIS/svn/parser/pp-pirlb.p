...
/* #4106 копируем op-impexp и msg-impexp с поступившего ПП на проводки з/п проекта */
{pfuncdef
   &NAME          = "PirCopyOP-IMPEXP"
   &DESCRIPTION   = "Копирует значения из строки в op-impexp из старого op в новый"
   &PARAMETERS    = "Строка из op-impexp, нужный op"
   &RESULT        = "LOGICAL"
   &SAMPLE        = "PirPeriodProc(ДАТА(),'01/13')"
   }
 ...
 if not available op-impexp then do:
	message "Не найдена соответствующая проводка!" view-as alert-box.
	out_Result = NO.
	end.
 else do:
	CREATE Top-impexp.
...
{pfuncdef
 ...
 DEF OUTPUT PARAM is-ok      AS INT  NO-UNDO. 		/* успешно ли выполнена ? */
	...
	if search("pir-clearinginitf.r") <> ? then
		...
		else message "Процедура  pir-clearinginitf не найдена!" view-as alert-box.
   is-ok = 0.
END PROCEDURE.

{pfuncdef
   &NAME          = "PirLinkLoan"
   &DESCRIPTION   = "Осуществляет привязку счета 9999*"
 ...
 DEF OUTPUT PARAM is-ok      AS INT  NO-UNDO. 		/* успешно ли выполнена ? */
	...
	if search("pir-linkloan.r") <> ? then
		...
		else message "Процедура  pir-linkloan не найдена!" view-as alert-box.
   is-ok = 0.
END PROCEDURE.

{pfuncdef
   &NAME          = "PirUnLinkLoan"
   &DESCRIPTION   = "Осуществляет отвязку счета 9999*"
   &PARAMETERS    = "iNum"
   &RESULT        = "CHAR"
 ...
 DEF OUTPUT PARAM is-ok      AS INT  NO-UNDO. 		/* успешно ли выполнена ? */
	...
	if search("pir-unlinkloan.r") <> ? then
		...
		else message "Процедура  pir-unlinkloan не найдена!" view-as alert-box.
   is-ok = 0.
END PROCEDURE.

{pfuncdef
   &NAME          = "PirGetCounter"
   &DESCRIPTION   = "Присваивает очередной номер из счетчика"
   &PARAMETERS    = "CHAR"
   &RESULT        = "INT64"
