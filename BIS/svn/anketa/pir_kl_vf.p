...
*/
    ...
    NO-ERROR.
    ttBeneficiaryPers.addressOfLaw = IF AVAIL(cust-ident) THEN Kladr(cust-ident.issue) ELSE ?.
    ...
    FIND FIRST cust-ident WHERE
         ...
         AND cust-ident.close-date     EQ ?
    NO-ERROR.
    ttBeneficiaryPers.addressOfStay = IF AVAIL(cust-ident) THEN Kladr(cust-ident.issue) ELSE ?.
