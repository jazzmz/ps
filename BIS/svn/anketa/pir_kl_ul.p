...
*/
    ...
    NO-ERROR.
    ttCorporation.addressOfStay = IF AVAIL(cust-ident) THEN Kladr(cust-ident.issue) ELSE ?.
    ...
    FIND FIRST cust-ident WHERE
         ...
         AND cust-ident.close-date     EQ ?
    NO-ERROR.
    ttCorporation.addressOfPost = IF AVAIL(cust-ident) THEN Kladr(cust-ident.issue) ELSE ?.
