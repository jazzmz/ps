...
{exp-path.i &exp-filename = "'analiz/grt_' + 
...
def var mPrRisk as dec no-undo.
   ...
   put screen col 1 row 24 color bright-blink-normal 
    ...
    FOR EACH loan WHERE ( open-date LE end-date ) no-lock:
        ...
        if (TRIM(cacct) NE "") THEN DO:
           ...
           mPrRisk = LnRsrvRate (loan.contract,loan.cont-code,end-date).
...
put screen col 1 row 24 color normal 
    ...
    FOR EACH gracc no-lock:
        IF gracc.GrRisk NE 0 THEN
           put unformatted skip gracc.acct FORMAT "x(25)" " " gracc.GrRisk FORMAT "9" " " gracc.prRisk FORMAT ">>9.99" " " end-date FORMAT "99.99.9999".
    END.
