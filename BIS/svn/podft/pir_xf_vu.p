...
*/
    ...
    NO-ERROR.
    ttBeneficiaryCorp.addressOfStay = IF AVAIL(cust-ident) THEN cust-ident.issue ELSE ?.
    ...
    FIND FIRST cust-ident WHERE
         ...
         AND cust-ident.close-date     EQ ?
    NO-ERROR.
    ttBeneficiaryCorp.addressOfPost = IF AVAIL(cust-ident) THEN cust-ident.issue ELSE ?.
