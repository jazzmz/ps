...
/******************************************* Присвоение значений переменным и др. */
    ...
    FOR LAST cust-ident
       ...
       NO-LOCK
    :
       cTmp = cust-ident.cust-code-type + ','
            + cust-ident.cust-code      + ','
            + STRING(cust-ident.cust-type-num).
       cTmp = GetXAttrValue("cust-ident", cTmp, "Подразд").
       ...
       ttPerson.document = GetCodeName("КодДокум", cust-ident.cust-code-type) + " "
          ...
          + ", " + cust-ident.issue
          + (IF (cTmp EQ "") THEN "" ELSE (", код подр." + cTmp)).
     END.
...
*/
    ...
    NO-ERROR.
    ttPerson.addressOfLaw = IF AVAIL(cust-ident) THEN cust-ident.issue ELSE ?.
    ...
    FIND FIRST cust-ident WHERE
         ...
         AND cust-ident.close-date     EQ ?
    NO-ERROR.
    ttPerson.addressOfStay = IF AVAIL(cust-ident) THEN cust-ident.issue ELSE ?.
    ...
    ttPerson.tel = TRIM(TRIM(person.phone[1], ",") + "," + TRIM(person.phone[2], ","), ",").
