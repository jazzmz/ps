...
FUNCTION GetLastHistoryDate RETURNS DATE
   ...
   DEFINE VARIABLE cKlList    AS CHARACTER INIT "ЮЛ,ИП,ФЛ,Б"                       NO-UNDO.
   DEFINE VARIABLE cTabList   AS CHARACTER INIT "cust-corp,cust-corp,person,banks" NO-UNDO.
   DEFINE VARIABLE cTipeList  AS CHARACTER INIT "Ю,Ю,Ч,Б"                          NO-UNDO.
   DEFINE VARIABLE cAdrList   AS CHARACTER EXTENT 4
                                 INIT ["АдрЮр,АдрФакт", "АдрЮр,АдрФакт,АдрПочт",
                                       "АдрПроп,АдрФакт", "АдрЮр,АдрФакт"]         NO-UNDO.
   DEFINE VARIABLE cFldList   AS CHARACTER EXTENT 4                                NO-UNDO.
   /*         ЮЛ      */
   cFldList[1] = "name-corp,name-short,*engl-name,cust-stat,*ОГРН,*RegDate,*RegPlace,"
               + "*tel,fax,inn,*ИИН,*СтруктОрг,*СоставСД,*УставКап,*ПрисутОргУправ,"
               + "*pirOtherBanks,*pirBusImage,*РискОтмыв,*ОценкаРиска,*FirstAccDate,"
               + "addr-of-low[1],addr-of-low[2],*PlaceOfStay".
   /*         ИП      */
   cFldList[2] = "name-corp,cust-stat,*ОГРН,*RegDate,*RegPlace,*tel,fax,inn,"
               + "*pirOtherBanks,*pirBusImage,*РискОтмыв,*ОценкаРиска,*FirstAccDate,"
   ...
   /*         ФЛ      */
   cFldList[3] = "name-last,first-names,phone,fax,*РискОтмыв,*ОценкаРиска,*FirstAccDate,"
   ...
   cFldList[4] = "name,*pirFullName,short-name,*pirShortName,*engl-name,*bank-stat,"
               + "*ОГРН,*RegDate,*RegPlace,*tel,*fax,*ИИН,*СтруктОрг,*УставКап,*ПрисутОргУправ,"
               + "*РискОтмыв,*ОценкаРиска,*FirstAccDate,"
               + "Юрид. адрес,Почт. адрес".
