   ...
   Parameters:
     ...
     Modified: 
     					 ...
     					 24.07.2006 10:42 Бурягин Е.П. - добавил условие, которое исключает из отчета 
     					                  ...
     					                  (только для кассы послеопреационного обслуживания)
     					                  В том случае, если по счету кассы нет других проводок кроме как 
     					                  аванса или возврата, то кассовый журнал все равно формируется с 
     					                  нулевым кол-вом строк.
  ...
  FOR EACH acct WHERE acct.contract begins "Касса"
    ...
    if suppress then do:
      find first op-entry where op-entry.acct-db eq acct.acct and
                                op-entry.op-status > "A"  /* >= gop-status */ and
 ...
 */
    ...
    /** Бурягин добавил условие 21.07.2006 15:50 */
    if not label_displayed and branch.branch-id = "00002" then
    	do:
	    	label_displayed = yes.
    	display  "┌───────────────────┐" AT 75 SKIP
    ...
    for each op-entry where op-entry.acct-db eq acct.acct
        ...
        /** Бурягин добавил поиск и условие 24.07.2006 10:45 */
		    if op-entry.acct-cr begins "20202" then do:
    			find first bfrAcct where bfrAcct.acct = op-entry.acct-cr no-lock.
...
/*            FRealSymbol (ROWID (op-entry), YES) column-label "СИМВОЛ!КАССОВОЙ!ОТЧЕТНОСТИ" */.
        ...
        IF AVAIL _user THEN username = _user._user-name.
       	ELSE username = "      НЕ ПРОВЕДЕНО ПО БАЛАНСУ".
       	
           down with frame cs.
           underline doc-count acct.acct doc-type.name-doc op.doc-num
                     op-entry.amt-cur with frame cs.
