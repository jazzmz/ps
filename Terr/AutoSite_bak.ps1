...
sleep 30
while($ie.ReadyState -ne 4) {start-sleep -m 100}
[Windows.Forms.Cursor]::Position = "$(635),$(270)"
[AutoClicker.Temp]::LeftClick()
#[AutoClicker.Temp]::RightClick()
sleep 5
...
$count++
if ($count -ge 2){sendmail "Совершено $count неудачных попыток. Завершение."; exit}
}
while (!$totalflag)
}
...
Get-Childitem "$dirTemp" -Filter "*.dbf" | % {
    ...
    if (!$lflag){
        ...
        SendMail "Закачан новый справочник $curName"
